...
def main(src, dst):
    if sys.platform == "nt":
        sys.stderr.write('icmpsh 主控端只能在 POSIX 系統上運行\n')
        sys.exit(255)
    ...
    try:
        ...
        sys.stderr.write('你需要先安裝 Python 的 Impacket 庫\n')
        sys.exit(255)
    ...
    try:
        ...
        sys.stderr.write('你需要以管理員權限運行 icmpsh 主控端\n')
        sys.exit(1)
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        msg = '缺少必要參數。請以 root 身份執行：\n'
        msg += './icmpsh-m.py <源 IP 地址> <目標 IP 地址>\n'
