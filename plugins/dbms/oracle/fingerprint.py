...
class Fingerprint(GenericFingerprint):
    ...
    def checkDbms(self):
        ...
        infoMsg = "測試 %s" % DBMS.ORACLE
        logger.info(infoMsg)

        # 注意:直接連接到 Oracle 數據庫時,SELECT LENGTH(SYSDATE)=LENGTH(SYSDATE) FROM DUAL 不起作用
        ...
        if result:
            ...
            if not result:
                warnMsg = "後端 DBMS 不是 %s" % DBMS.ORACLE
