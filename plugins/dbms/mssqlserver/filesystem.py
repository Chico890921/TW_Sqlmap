...
class Filesystem(GenericFilesystem):
    ...
    def _updateDestChunk(self, fileContent, tmpPath):
        ...
        logger.debug("正在將調試腳本上傳到 %s\\%s,請稍候.." % (tmpPath, randScr))
    ...
    def unionWriteFile(self, localFile, remoteFile, fileType, forceCheck=False):
        errMsg = "Microsoft SQL Server 不支持使用 UNION 查詢 SQL 注入技術進行文件上傳"
        #errMsg += "UNION query SQL injection technique"
        raise SqlmapUnsupportedFeatureException(errMsg)

    def _stackedWriteFilePS(self, tmpPath, localFileContent, remoteFile, fileType):
        infoMsg = "使用 PowerShell 將%s 文件內容寫入文件'%s'" % (fileType, remoteFile)
    ...
    def _stackedWriteFileDebugExe(self, tmpPath, localFile, localFileContent, remoteFile, fileType):
        infoMsg = "使用 debug.exe 將%s 文件內容寫入文件'%s',請稍候.." % (fileType, remoteFile)
        ...
        if localFileSize < debugSize:
            ...
            debugMsg = "將分塊文件 %s\\%s 重命名為 %s 文件 %s\\%s,並將其移動到 %s" % (tmpPath, chunkName, fileType, tmpPath, remoteFileName, remoteFile)
            ...
            debugMsg = "文件大小超過 %d 字節。sqlmap 將在本地將其拆分為分塊,逐個分塊上傳,並在服務器上重新創建原始文件,請稍候.." % debugSize
            ...
            logger.debug("將 %s 文件 %s 移動到 %s" % (fileType, sFile, remoteFile))
    ...
    def _stackedWriteFileVbs(self, tmpPath, localFileContent, remoteFile, fileType):
        infoMsg = "使用自定義的 Visual Basic 腳本將%s 文件內容寫入文件'%s',請稍候.." % (fileType, remoteFile)
    ...
    def _stackedWriteFileCertutilExe(self, tmpPath, localFile, localFileContent, remoteFile, fileType):
        infoMsg = "使用 certutil.exe 將%s 文件內容寫入文件'%s',請稍候.." % (fileType, remoteFile)
        #infoMsg += "file content to file '%s', please wait.." % remoteFile
        logger.info(信息)
    ...
    def stackedWriteFile(self, localFile, remoteFile, fileType, forceCheck=False):
        ...
        if written is False:
            message = "是否嘗試使用內置的 certutil.exe 技術上傳文件？[Y/n] "
