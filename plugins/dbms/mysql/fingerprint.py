...
class Fingerprint(GenericFingerprint):
    ...
    def _commentCheck(self):
        infoMsg = "執行 %s 註釋注入指紋識別" % DBMS.MYSQL
        logger.info(infoMsg)

        result = inject.checkBooleanExpression("[RANDNUM]=[RANDNUM]/* NoValue */")

        if not result:
            warnMsg = "無法執行 %s 註釋注入" % DBMS.MYSQL
            logger.warning(warnMsg)

            return None

        # 參考:https://downloads.mysql.com/archives/community/
        # 參考:https://dev.mysql.com/doc/relnotes/mysql/<major>.<minor>/en/
    ...
    def getFingerprint(self):
        ...
        if kb.data.banner:
            dbmsOsFp = Format.getOs("後端 DBMS", kb.bannerFp)
    ...
    def checkDbms(self):
        ...
        * http://dev.mysql.com/doc/refman/6.0/en/news-6-0-x.html (manual has been withdrawn)
        ...
        if result:
            ...
            if not result:
                warnMsg = "後端 DBMS 不是 %s" % DBMS.MYSQL
                logger.warning(warnMsg)

                return False

            # 在某些平臺上讀取 information_schema 會導致煩人的超時退出
            # 參考:http://bugs.mysql.com/bug.php?id=15855
    ...
    def checkDbmsOs(self, detailed=False):
        ...
        if Backend.getOs():
            infoMsg = "後端 DBMS 操作系統為 %s" % Backend.getOs()
            logger.info(infoMsg)
        else:
