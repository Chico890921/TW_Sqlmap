...
class Takeover(GenericTakeover):
    def udfSetRemotePath(self):
        ...
        if Backend.isOs(OS.WINDOWS):
            # DLL 可以在任何 postgres 用戶具有讀/寫/執行訪問權限的文件夾中
        ...
        # 在 Linux 上
            ...
            # SO 可以在任何 postgres 用戶具有讀/寫/執行訪問權限的文件夾中
            # read/write/execute access is valid
    ...
    def udfCreateFromSharedLib(self, udf, inpRet):
        if udf in self.udfToCreate:
            logger.info("從二進制 UDF 文件創建 UDF '%s'" % udf)
            ...
            logger.debug("按要求保留現有的 UDF '%s'" % udf)
    ...
    def copyExecCmd(self, cmd):
        ...
        if isStackingAvailable():
            # 參考:https://medium.com/greenwolf-security/authenticated-arbitrary-command-execution-on-postgresql-9-3-latest-cd18945914d5
