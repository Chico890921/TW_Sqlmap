...
class Entries(object):
    ...
    def dumpTable(self, foundData=None):
        ...
        if conf.db is None or conf.db == CURRENT_DB:
            if conf.db is None:
                warnMsg = "缺少數據庫參數。sqlmap 將使用當前數據庫來枚舉表條目"
        ...
        if conf.tbl:
            ...
            if len(kb.data.cachedTables) > 0:
                ...
                errMsg = "無法檢索數據庫'%s'中的表" % unsafeSQLIdentificatorNaming(conf.db)
        ...
        for tbl in tblList:
            ...
            if conf.exclude and re.search(conf.exclude, unsafeSQLIdentificatorNaming(tbl), re.I) is not None:
                infoMsg = "跳過表'%s'" % unsafeSQLIdentificatorNaming(tbl)
                singleTimeLogMessage(infoMsg)
                continue
            ...
            try:
                ...
                if safeSQLIdentificatorNaming(conf.db) not in kb.data.cachedColumns or safeSQLIdentificatorNaming(tbl, True) not in kb.data.cachedColumns[safeSQLIdentificatorNaming(conf.db)] or not kb.data.cachedColumns[safeSQLIdentificatorNaming(conf.db)][safeSQLIdentificatorNaming(tbl, True)]:
                    warnMsg = "無法枚舉表'%s'的列" % unsafeSQLIdentificatorNaming(tbl)
                    if METADB_SUFFIX.upper() not in conf.db.upper():
                        warnMsg += "在數據庫'%s'中" % unsafeSQLIdentificatorNaming(conf.db)
                    warnMsg += ",跳過"
                    logger.warning(warnMsg)
                ...
                if not colList:
                    warnMsg = "跳過表'%s'" % unsafeSQLIdentificatorNaming(tbl)
                    if METADB_SUFFIX.upper() not in conf.db.upper():
                        warnMsg += "在數據庫'%s'中" % unsafeSQLIdentificatorNaming(conf.db)
                    warnMsg += "(沒有可用的列名)"
                    logger.warning(warnMsg)
                    ...
                    if Backend.getIdentifiedDbms() in (DBMS.ORACLE, DBMS.DB2, DBMS.DERBY, DBMS.ALTIBASE, DBMS.MIMERSQL):
                        ...
                        if not (isTechniqueAvailable(PAYLOAD.TECHNIQUE.UNION) and kb.injection.data[PAYLOAD.TECHNIQUE.UNION].where == PAYLOAD.WHERE.ORIGINAL):
                            ...
                            if Backend.isDbms(DBMS.MSSQL) and not conf.forcePivoting:
                                warnMsg = "在錶轉儲出現問題時 (例如列條目順序),建議您重新運行使用'--force-pivoting'"
                ...
                if not kb.data.dumpedTable and isInferenceAvailable() and not conf.direct:
                    ...
                    infoMsg += "在數據庫'%s'" % unsafeSQLIdentificatorNaming(conf.db)
                    logger.info(infoMsg)
                    ...
                    if count == 0:
                        ...
                        warnMsg += "在數據庫'%s' " % unsafeSQLIdentificatorNaming(conf.db)
                        warnMsg += "似乎為空"
                        logger.warning(warnMsg)
                ...
                if len(kb.data.dumpedTable) == 0 or (entriesCount == 0 and kb.permissionFlag):
                    ...
                    if conf.col:
                        warnMsg += "列'%s' " % colNames
                    warnMsg += "在表'%s' " % unsafeSQLIdentificatorNaming(tbl)
                    warnMsg += "在數據庫'%s'%s" % (unsafeSQLIdentificatorNaming(conf.db), "(權限被拒絕)" if kb.permissionFlag else "")
                    logger.warning(warnMsg)
                else:
                    ...
                    if not conf.disableHashing:
                        try:
                            ...
                            errMsg = "在攻擊錶轉儲時發生錯誤 ('%s')" % getSafeExString(ex)
                ...
                errMsg = "在轉儲階段檢測到連接異常 ('%s')" % getSafeExString(ex)
    ...
    def dumpAll(self):
        ...
        if Backend.isDbms(DBMS.MYSQL) and not kb.data.has_information_schema:
            errMsg = "information_schema 不可用,後端 DBMS 是 MySQL < 5.0"
            #errMsg += "back-end DBMS is MySQL < 5.0"
            raise SqlmapUnsupportedFeatureException(errMsg)

        infoMsg = "sqlmap 現在將轉儲所有數據庫中的所有表的條目"
        logger.info(infoMsg)
            ...
            for db, tables in kb.data.cachedTables.items():
                ...
                for table in tables:
                    if conf.exclude and re.search(conf.exclude, table, re.I) is not None:
                        infoMsg = "跳過表'%s'" % unsafeSQLIdentificatorNaming(table)
                        logger.info(infoMsg)
                        continue
