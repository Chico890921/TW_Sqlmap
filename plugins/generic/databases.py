...
class Databases(object):
    ...
    def getDbs(self):
        ...
        if Backend.isDbms(DBMS.MYSQL) and not kb.data.has_information_schema:
            warnMsg = "information_schema 不可用,後端 DBMS 是 MySQL < 5.0"
            ...
            warnMsg = "架構名稱將在%s 上用於枚舉,作為其他 DBMS 上數據庫名稱的對應項" % Backend.getIdentifiedDbms()
            ...
            warnMsg = "用戶名稱將用於%s 上的枚舉,作為其他 DBMS 上數據庫名稱的對應項" % Backend.getIdentifiedDbms()
            #warnMsg += "for enumeration as the counterpart to database "
            #warnMsg += "names on other DBMSes"
            logger.warning(warnMsg)
    ...
    def getTables(self, bruteForce=None):
        ...
        if bruteForce is None:
            if Backend.isDbms(DBMS.MYSQL) and not kb.data.has_information_schema:
                ...
                if not tables:
                    warnMsg = "無法獲取表名稱,後端 DBMS 是%s" % Backend.getIdentifiedDbms()
        ...
        if bruteForce:
            ...
            message = "是否使用常見數據表存在性檢查？ %s " % ("[Y/n/q]" if Backend.getIdentifiedDbms() in (DBMS.ACCESS, DBMS.MCKOI, DBMS.EXTREMEDB) else "[y/N/q]")
                ...
                if condition:
                    if not Backend.isDbms(DBMS.SQLITE):
                        ...
                        if conf.excludeSysDbs:
                            infoMsg = "跳過系統數據庫%s:%s" % ("s" if len(self.excludeDbsList) > 1 else "", ", ".join(unsafeSQLIdentificatorNaming(db) for db in self.excludeDbsList))
                            logger.info(infoMsg)
                        ...
                        if conf.getComments:
                            ...
                            if hasattr(_, "query"):
                                ...
                                if not isNoneValue(comment):
                                    infoMsg = "獲取表格'%s'的註釋'%s'" % (unsafeSQLIdentificatorNaming(table), comment)
                                    if METADB_SUFFIX not in db:
                                        infoMsg += " in database '%s'" % unsafeSQLIdentificatorNaming(db)
                                    logger.info(infoMsg)
                            else:
                                warnMsg = "在%s 上無法獲取列註釋" % Backend.getIdentifiedDbms()
        ...
        if not kb.data.cachedTables and isInferenceAvailable() and not conf.direct:
            for db in dbs:
                if conf.excludeSysDbs and db in self.excludeDbsList:
                    infoMsg = "跳過系統數據庫'%s'" % unsafeSQLIdentificatorNaming(db)
                    logger.info(infoMsg)
                    continue

                if conf.exclude and re.search(conf.exclude, db, re.I) is not None:
                    infoMsg = "跳過數據庫'%s'" % unsafeSQLIdentificatorNaming(db)
                ...
                for _query, _count in ((rootQuery.blind.query, rootQuery.blind.count), (getattr(rootQuery.blind, "query2", None), getattr(rootQuery.blind, "count2", None))):
                    ...
                    infoMsg = "獲取數據庫'%s'的表數量" % unsafeSQLIdentificatorNaming(db)
                    ...
                    if count == 0:
                        warnMsg = "數據庫'%s'似乎為空" % unsafeSQLIdentificatorNaming(db)
                        #warnMsg += "appears to be empty"
                        logger.warning(warnMsg)
                        break

                    elif not isNumPosStrValue(count):
                        warnMsg = "無法獲取數據庫'%s'的表數量" % unsafeSQLIdentificatorNaming(db)
                    ...
                    if tables:
                        ...
                        warnMsg = "無法獲取數據庫 '%s' 的數據表名稱" % unsafeSQLIdentificatorNaming(db)
    ...
    def getColumns(self, onlyColNames=False, colTuple=None, bruteForce=None, dumpMode=False):
        ...
        if conf.db is None or conf.db == CURRENT_DB:
            if conf.db is None:
                warnMsg = "缺少數據庫參數。sqlmap 將使用當前數據庫來枚舉表格列"
        ...
        if conf.tbl:
            ...
            if len(kb.data.cachedTables) > 0:
                ...
                if METADB_SUFFIX not in conf.db:
                    errMsg += "在數據庫'%s'中" % unsafeSQLIdentificatorNaming(conf.db)
                raise SqlmapNoneDataException(errMsg)
            else:
        ...
        if bruteForce is None:
            if Backend.isDbms(DBMS.MYSQL) and not kb.data.has_information_schema:
                ...
                warnMsg = "無法獲取列名稱,後端 DBMS 是%s" % Backend.getIdentifiedDbms()
                #warnMsg += "back-end DBMS is %s" % Backend.getIdentifiedDbms()
                singleTimeWarnMessage(warnMsg)
                bruteForce = True
        ...
        if bruteForce:
            ...
            if kb.choices.columnExists is None:
                message = "是否使用常見字段存在性檢查？ %s" % ("[Y/n/q]" if Backend.getIdentifiedDbms() in (DBMS.ACCESS, DBMS.MCKOI, DBMS.EXTREMEDB) else "[y/N/q]")
            ...
            for tbl in tblList:
                if conf.db is not None and len(kb.data.cachedColumns) > 0 \
                   ...
                   kb.data.cachedColumns[conf.db]:
                    infoMsg = "從數據庫'%s'中獲取了表格列信息" % unsafeSQLIdentificatorNaming(conf.db)
                ...
                if dumpMode and colList:
                    ...
                    iinfoMsg += "用於數據表 '%s' " % unsafeSQLIdentificatorNaming(tbl)
                    if METADB_SUFFIX not in conf.db:
                        infoMsg += "在數據庫 '%s' 中" % unsafeSQLIdentificatorNaming(conf.db)
                    logger.info(infoMsg)
                            ...
                            if name:
                                if conf.getComments:
                                    ...
                                    if hasattr(_, "query"):
                                        ...
                                        comment = unArrayizeValue(inject.getValue(query, blind=False, time=False))
                                        if not isNoneValue(comment):
                                            infoMsg = "獲取列'%s'的註釋'%s'" % (name, comment)
                                            logger.info(infoMsg)
                                    else:
                                        warnMsg = "在%s 上無法獲取列註釋" % Backend.getIdentifiedDbms()
            ...
            for tbl in tblList:
                ...
                if dumpMode and colList:
                    ...
                    if not isNumPosStrValue(count):
                        ...
                        if not columns:
                            errMsg = "無法獲取表格'%s'的%s 列信息" % (unsafeSQLIdentificatorNaming(tbl), "數量" if not Backend.isDbms(DBMS.MSSQL) else "")
                    ...
                    if not isNoneValue(column):
                        if conf.getComments:
                            ...
                            if hasattr(_, "query"):
                                ...
                                comment = unArrayizeValue(inject.getValue(query, union=False, error=False))
                                if not isNoneValue(comment):
                                    infoMsg = "獲取列'%s'的註釋'%s'" % (column, comment)
                                    logger.info(infoMsg)
                            else:
                                warnMsg = "在%s 上無法獲取列註釋" % Backend.getIdentifiedDbms()
                                #warnMsg += "possible to get column comments"
        ...
        if not kb.data.cachedColumns:
            warnMsg = "無法獲取表格'%s'的列名稱" % unsafeSQLIdentificatorNaming(unArrayizeValue(tblList)) if len(tblList) == 1 else "無法獲取任何表格的列名稱"
    ...
    def getCount(self):
        if not conf.tbl:
            warnMsg = "缺少表格參數,sqlmap 將獲取所有數據庫管理系統數據庫表格的條目數量"
        ...
        if conf.tbl is not None and conf.db is None and Backend.getIdentifiedDbms() not in (DBMS.SQLITE, DBMS.ACCESS, DBMS.FIREBIRD, DBMS.MCKOI, DBMS.EXTREMEDB):
            warnMsg = "缺少數據庫參數。sqlmap 將使用當前數據庫來獲取表格'%s'的條目數量" % unsafeSQLIdentificatorNaming(conf.tbl)
