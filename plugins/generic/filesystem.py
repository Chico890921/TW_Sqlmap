...
class Filesystem(object):
    ...
    def _checkFileLength(self, localFile, remoteFile, fileRead=False):
        ...
        if fileRead and Backend.isDbms(DBMS.PGSQL):
            logger.info("無法在 PostgreSQL 上檢查讀取的文件 '%s' 的長度" % remoteFile)
            sameFile = True
        else:
            logger.debug("檢查遠程文件 '%s' 的長度" % remoteFile)
            ...
            if isNumPosStrValue(remoteFileSize):
                ...
                if localFileSize == remoteFileSize:
                    sameFile = True
                    infoMsg = "本地文件 '%s' 和遠程文件 " % localFile
                    infoMsg += "'%s' 的大小相同 (%d B)" % (remoteFile, localFileSize)
                elif remoteFileSize > localFileSize:
                    infoMsg = "遠程文件 '%s' 較大 (%d B)" % (remoteFile, remoteFileSize)
                    infoMsg += "比本地文件 '%s' (%dB)" % (localFile, localFileSize)
                else:
                    infoMsg = "遠程文件 '%s' 較小 (%d B)" % (remoteFile, remoteFileSize)
                    infoMsg += "比文件 '%s' (%d B)" % (localFile, localFileSize)

                logger.info(infoMsg)
            else:
                sameFile = False
                warnMsg = "看起來文件還沒有被寫入 (通常是因為 DBMS 進程用戶在目標路徑上沒有寫入權限)"
    ...
    def fileToSqlQueries(self, fcEncodedList):
        """
        被 MySQL 和 PostgreSQL 插件調用,以在後端 DBMS 的文件系統上寫入文件
    ...
    def askCheckWrittenFile(self, localFile, remoteFile, forceCheck=False):
        ...
        if forceCheck is not True:
            message = "你是否要確認本地文件 '%s' " % localFile
            message += "已成功寫入後端 DBMS "
            message += "文件系統 ('%s')？ [Y/n] " % remoteFile
    ...
    def askCheckReadFile(self, localFile, remoteFile):
        if not kb.bruteMode:
            message = "是否需要確認遠程文件 '%s' 已成功從後端數據庫文件系統下載？[Y/n] " % remoteFile
    ...
    def nonStackedReadFile(self, remoteFile):
        errMsg = "'nonStackedReadFile' 方法必須在特定的 DBMS 插件中定義"
        #errMsg += "into the specific DBMS plugin"
        raise SqlmapUndefinedMethod(errMsg)

    def stackedReadFile(self, remoteFile):
        errMsg = "'stackedReadFile' 方法必須在特定的 DBMS 插件中定義"
        #errMsg += "into the specific DBMS plugin"
        raise SqlmapUndefinedMethod(errMsg)

    def unionWriteFile(self, localFile, remoteFile, fileType, forceCheck=False):
        errMsg = "'unionWriteFile' 方法必須在特定的 DBMS 插件中定義"
        #errMsg += "into the specific DBMS plugin"
        raise SqlmapUndefinedMethod(errMsg)

    def stackedWriteFile(self, localFile, remoteFile, fileType, forceCheck=False):
        errMsg = "'stackedWriteFile' 方法必須在特定的 DBMS 插件中定義"
        #errMsg += "into the specific DBMS plugin"
        raise SqlmapUndefinedMethod(errMsg)
    ...
    def writeFile(self, localFile, remoteFile, fileType=None, forceCheck=False):
        ...
        if conf.direct or isStackingAvailable():
            if isStackingAvailable():
                debugMsg = "使用 LINES TERMINATED BY 技術上傳文件 '%s'" % fileType
