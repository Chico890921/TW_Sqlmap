...
class Users(object):
    ...
    def getPasswordHashes(self):
            ...
            if Backend.isDbms(DBMS.SYBASE):
                ...
                for user in users:
                    ...
                    if Backend.getIdentifiedDbms() in (DBMS.INFORMIX, DBMS.VIRTUOSO):
                        ...
                        if not isNumPosStrValue(count):
                            warnMsg = "無法獲取用戶 '%s' 的密碼哈希值數量" % user
                            #warnMsg += "hashes for user '%s'" % user
                            logger.warning(warnMsg)
                            continue

                    infoMsg = "獲取用戶 '%s' 的密碼哈希值" % user
                    logger.info(infoMsg)
        ...
        if not kb.data.cachedUsersPasswords:
            ...
            message = "是否要對獲取到的密碼哈希值進行字典攻擊？ [Y/n/q]"
                ...
                for value in values:
                    ...
                    for count in xrange(0, len(value or [])):
                        ...
                        # 其他列是權限
                            ...
                            # 在 PostgreSQL 中,如果權限為 True,則得到 1,否則得到 0
                            # True, 0 otherwise
            ...
            for user in users:
                ...
                if Backend.isDbms(DBMS.INFORMIX):
                    ...
                    if not isNumPosStrValue(count):
                        ...
                        warnMsg = "無法獲取用戶 '%s' 的權限數量" % outuser
                        #warnMsg += "privileges for user '%s'" % outuser
                        logger.warning(warnMsg)
                        continue

                infoMsg = "獲取用戶 '%s' 的權限" % outuser
                logger.info(infoMsg)
                ...
                for index in indexRange:
                    ...
                    # 在 MySQL < 5.0 中,第一次獲取用戶的權限後,我們中斷循環,否則會重複相同的查詢
    ...
    def getRoles(self, query2=False):
        warnMsg = "在 %s 上,角色的概念不存在。sqlmap 將枚舉權限而不是角色" % Backend.getIdentifiedDbms()
