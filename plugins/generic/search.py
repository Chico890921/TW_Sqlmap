...
class Search(object):
    ...
    def searchDb(self):
        ...
        for db in dbList:
            ...
            if conf.excludeSysDbs:
                ...
                infoMsg = "跳過系統數據庫%s '%s'" % ("s" if len(self.excludeDbsList) > 1 else "", ", ".join(db for db in self.excludeDbsList))
                logger.info(infoMsg)
            else:
                exclDbsQuery = ""
        ...
        conf.dumper.lister("找到的數據庫", foundDbs)
    ...
    def searchTable(self):
        ...
        if Backend.isDbms(DBMS.MYSQL) and not kb.data.has_information_schema:
            ...
            errMsg += "後端 DBMS 是 MySQL < 5.0"
            bruteForce = True

        if bruteForce:
            message = "是否要使用常見的表存在性檢查？%s" % ("[Y/n/q]" if Backend.getIdentifiedDbms() in (DBMS.ACCESS, DBMS.MCKOI, DBMS.EXTREMEDB) else "[y/N/q]")
        ...
        for tbl in tblList:
            ...
            if dbCond and conf.db:
                ...
                msg = "跳過系統數據庫%s '%s'" % ("s" if len(self.excludeDbsList) > 1 else "", ", ".join(db for db in self.excludeDbsList))
                logger.info(msg)
            else:
                whereDbsQuery = ""
            ...
            if not values and isInferenceAvailable() and not conf.direct:
                ...
                for db in foundTbls:
                    ...
                    infoMsg = "正在獲取數據庫 '%s' 中包含表的數量" % unsafeSQLIdentificatorNaming(db)
                    if tblConsider == "1":
                        infoMsg += "s LIKE"
    ...
    def searchColumn(self):
        ...
        if bruteForce:
            message = "是否要使用通用字段存在性檢查？ %s" % ("[Y/n/q]" if Backend.getIdentifiedDbms() in (DBMS.ACCESS, DBMS.MCKOI, DBMS.EXTREMEDB) else "[y/N/q]")
            choice = readInput(message, default='Y' if 'Y' in message else 'N').upper()
        ...
        for column in colList:
            ...
            if dbCond:
                if conf.db:
                    ...
                    msg = "跳過系統數據庫%s '%s'" % ("s" if len(self.excludeDbsList) > 1 else "", ", ".join(unsafeSQLIdentificatorNaming(db) for db in self.excludeDbsList))
                    logger.info(msg)
                else:
                    infoMsgDb = " 跨所有數據庫"
    ...
    def search(self):
        ...
        if conf.col:
            ...
            errMsg = "缺少參數,請提供 -D、-T 或 -C 以及 --search"
            #errMsg += "with --search"
