...
class Custom(object):
    ...
    def sqlQuery(self, query):
        ...
        try:
            ...
            if not re.search(r"\b(OPENROWSET|INTO)\b", query, re.I) and (not sqlType or "SELECT" in sqlType):
                infoMsg = "獲取 %s 查詢結果: '%s'" % (sqlType if sqlType is not None else "SQL", query)
                logger.info(infoMsg)
                ...
                warnMsg = "只有在支持堆疊查詢時才能執行非查詢 SQL 語句"
                ...
                if sqlType:
                    infoMsg = "執行 %s 語句: '%s'" % (sqlType if sqlType is not None else "SQL", query)
                else:
                    infoMsg = "執行未知的 SQL 命令: '%s'" % query
                logger.info(infoMsg)
    ...
    def sqlShell(self):
        infoMsg = "調用 %s shell。輸入 'x' 或 'q' 退出" % Backend.getIdentifiedDbms()
