...
def main():
    ...
    apiparser.add_argument("-s", "--server", help="作為 REST-JSON API 服務器運行", action="store_true")
    apiparser.add_argument("-c", "--client", help="作為 REST-JSON API 客戶端運行", action="store_true")
    ...
    apiparser.add_argument("-p", "--port", help="REST-JSON API 服務器的端口 (默認為%d)" % RESTAPI_DEFAULT_PORT, default=RESTAPI_DEFAULT_PORT, type=int)
    apiparser.add_argument("--adapter", help="要使用的服務器 (bottle) 適配器 (默認為\"%s\")" % RESTAPI_DEFAULT_ADAPTER, default=RESTAPI_DEFAULT_ADAPTER)
    apiparser.add_argument("--database", help="設置 IPC 數據庫文件路徑 (可選)")
    apiparser.add_argument("--username", help="基本身份驗證用戶名 (可選)")
    apiparser.add_argument("--password", help="基本身份驗證密碼 (可選)")
