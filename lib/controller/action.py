...
def action():
    ...
    if not Backend.getDbms() or not conf.dbmsHandler:
        ...
        errMsg = "sqlmap 無法對後端數據庫管理系統進行指紋識別"
        # errMsg += "back-end database management system"

        if htmlParsed:
            errMsg = "但是通過 HTML 錯誤頁面,可以確定後端數據庫管理系統是%s" % htmlParsed
            # errMsg += "possible to determinate that the "
            # errMsg += "back-end DBMS is %s" % htmlParsed

        if htmlParsed and htmlParsed.lower() in SUPPORTED_DBMS:
            errMsg += "。請不要手動指定後端數據庫管理系統,sqlmap 將為您進行指紋識別"
            # errMsg += "sqlmap will fingerprint the DBMS for you"
        elif kb.nullConnection:
            errMsg += "。您可以嘗試重新運行,不使用優化開關'%s'" % ("-o" if conf.optimize else "--null-connection")
    ...
    if conf.getPasswordHashes:
        try:
            conf.dumper.userSettings("數據庫管理系統用戶密碼哈希值", conf.dbmsHandler.getPasswordHashes(), "密碼哈希值", CONTENT_TYPE.PASSWORDS)
    ...
    if conf.getPrivileges:
        try:
            conf.dumper.userSettings("數據庫管理系統用戶權限", conf.dbmsHandler.getPrivileges(), "權限", CONTENT_TYPE.PRIVILEGES)
        except SqlmapNoneDataException as ex:
            logger.critical(ex)
        except:
            raise
    ...
    if conf.getRoles:
        try:
            conf.dumper.userSettings("數據庫管理系統用戶角色", conf.dbmsHandler.getRoles(), "角色", CONTENT_TYPE.ROLES)
        except SqlmapNoneDataException as ex:
            logger.critical(ex)
        except:
            raise
