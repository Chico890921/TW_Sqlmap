...
class Replication(object):
    ...
    def __init__(self, dbpath):
        try:
            ...
            errMsg = "打開復制文件 '%s' 時發生錯誤 ('%s')" % (dbpath, getSafeExString(ex))
    ...
    class Table(object):
        ...
        def __init__(self, parent, name, columns=None, create=True, typeless=False):
            ...
            if create:
                try:
                    ...
                    errMsg = "在初始化位於 '%s' 的 SQLite 數據庫時發生問題 ('%s')" % (self.parent.dbpath, getSafeExString(ex, UNICODE_ENCODING))
        ...
        def execute(self, sql, parameters=None):
            try:
                ...
                errMsg = "訪問位於 '%s' 的 SQLite 數據庫時發生問題 ('%s')。請確保它沒有被其他程序使用" % (self.parent.dbpath, getSafeExString(ex, UNICODE_ENCODING))
                er#errMsg += "located at '%s'. Please make sure that " % self.parent.dbpath
