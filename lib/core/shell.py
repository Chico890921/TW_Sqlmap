...
def saveHistory(completion=None):
    try:
        ...
        try:
            ...
            warnMsg = "寫入歷史文件 '%s' 時出現問題 (%s)" % (historyPath, getSafeExString(ex))
...
def loadHistory(completion=None):
    ...
    if os.path.exists(historyPath):
        try:
            ...
            warnMsg = "加載歷史文件 '%s' 時出現問題 (%s)" % (historyPath, getSafeExString(ex))
            logger.warning(warnMsg)
        except UnicodeError:
            if IS_WIN:
                warnMsg = "加載歷史文件 '%s' 時出現問題。" % historyPath
                warnMsg += "更多信息請參考 'https://github.com/pyreadline/pyreadline/issues/30'"
                logger.warning(warnMsg)
