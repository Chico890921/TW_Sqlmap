...
def _setRequestParams():
    ...
    if conf.data is not None:
        ...
        if kb.processUserMarks is None and kb.customInjectionMark in conf.data:
            message = "在 %s 請求體中找到自定義注入標記 ('%s')。你想要處理它嗎？[Y/n/q] " % (conf.method, kb.customInjectionMark)
        ...
        if re.search(JSON_RECOGNITION_REGEX, conf.data):
            message = "在 %s 請求體中找到 JSON 數據。你想要處理它嗎？[Y/n/q] " % conf.method
            ...
            message = "在 %s 請求體中找到類似 JSON 的數據。你想要處理它嗎？[Y/n/q] " % conf.method
            #message += "Do you want to process it? [Y/n/q] "
            choice = readInput(message, default='Y').upper()
            ...
            message = "在 %s 請求體中找到類似數組的數據。你想要處理它嗎？[Y/n/q] " % conf.method
            #message += "Do you want to process it? [Y/n/q] "
            choice = readInput(message, default='Y').upper()
            ...
            message = "在 %s 請求體中找到 SOAP/XML 數據。你想要處理它嗎？[Y/n/q] " % conf.method
            #message += "Do you want to process it? [Y/n/q] "
            choice = readInput(message, default='Y').upper()
            ...
            message = "在 %s 請求體中找到類似多部分的數據。你想要處理它嗎？[Y/n/q] " % conf.method
            #message += "Do you want to process it? [Y/n/q] "
            choice = readInput(message, default='Y').upper()
    ...
    if re.search(URI_INJECTABLE_REGEX, conf.url, re.I) and not any(place in conf.parameters for place in (PLACE.GET, PLACE.POST)) and not kb.postHint and kb.customInjectionMark not in (conf.data or "") and conf.url.startswith("http"):
        warnMsg = "您提供的目標 URL 沒有任何 GET 參數 (例如 'http://www.site.com/article.php?id=1'),也沒有通過選項 '--data' 提供任何 POST 參數"
        #warnMsg += "parameters (e.g. 'http://www.site.com/article.php?id=1') "
        #warnMsg += "and without providing any POST parameters "
        #warnMsg += "through option '--data'"
        logger.warning(warnMsg)

        message = "您是否要嘗試在目標 URL 本身進行 URI 注入？ [Y/n/q] "
    ...
    for place, value in ((PLACE.URI, conf.url), (PLACE.CUSTOM_POST, conf.data), (PLACE.CUSTOM_HEADER, str(conf.httpHeaders))):
        ...
        if kb.customInjectionMark in _:
            if kb.processUserMarks is None:
                ...
                message = "在選項 '%s' 中找到自定義注入標記 ('%s')。你想要處理它嗎？[Y/n/q] " % (lut[place], kb.customInjectionMark)
                ...
                if choice == 'Q':
                    ...
                    if kb.processUserMarks:
                        ...
                        if "=%s" % kb.customInjectionMark in _:
                            warnMsg = "看起來您提供了空的參數值進行測試。請始終只使用有效的參數值,以便 sqlmap 能夠正常運行"
    ...
    if conf.csrfToken:
        if not any(re.search(conf.csrfToken, ' '.join(_), re.I) for _ in (conf.paramDict.get(PLACE.GET, {}), conf.paramDict.get(PLACE.POST, {}), conf.paramDict.get(PLACE.COOKIE, {}))) and not re.search(r"\b%s\b" % conf.csrfToken, conf.data or "") and conf.csrfToken not in set(_[0].lower() for _ in conf.httpHeaders) and conf.csrfToken not in conf.paramDict.get(PLACE.COOKIE, {}) and not all(re.search(conf.csrfToken, _, re.I) for _ in conf.paramDict.get(PLACE.URI, {}).values()):
            errMsg = "未在提供的 GET、POST、Cookie 或標頭值中找到反-CSRF 令牌參數 '%s'" % conf.csrfToken._original
        ...
        for place in (PLACE.GET, PLACE.POST, PLACE.COOKIE):
            ...
            for parameter in conf.paramDict.get(place, {}):
                if any(parameter.lower().count(_) for _ in CSRF_TOKEN_PARAMETER_INFIXES):
                    message = "%s 參數 '%s' 似乎包含反-CSRF 令牌。您是否希望 sqlmap 在後續請求中自動更新它？ [y/N] " % ("%s " % place if place != parameter else "", parameter)
    ...
    if conf.flushSession:
        if os.path.exists(conf.hashDBFile):
            ...
            try:
                ...
                logger.info("正在清空會話文件")
            except OSError as ex:
                errMsg = "無法刷新會話文件 ('%s')" % getSafeExString(ex)
...
def _resumeDBMS():
    ...
    if conf.dbms:
        ...
        if not check:
            message = "你提供了 '%s' 作為後端 DBMS," % conf.dbms
            message += "但從過去的掃描信息中,sqlmap 假設後端 DBMS 是 '%s'。" % dbms
            message += "你真的想要強制指定後端 DBMS 的值嗎？[y/N] "
...
def _resumeOS():
    ...
    if os and os != 'None':
        infoMsg = "恢復後端數據庫管理系統操作系統 '%s'" % os
        logger.info(infoMsg)

        if conf.os and conf.os.lower() != os.lower():
            message = "您提供的後端數據庫管理系統操作系統為 '%s',但根據過去的掃描信息,sqlmap 推測後端數據庫管理系統操作系統為 %s。您是否確定要強制指定後端數據庫管理系統操作系統的值？ [y/N] " % (conf.os, os)
...
def _setResultsFile():
    ...
    if not conf.resultsFP:
        ...
        try:
            ...
            try:
                warnMsg = "無法創建結果文件 '%s' ('%s')。" % (conf.resultsFile, getUnicode(ex))
                ...
                warnMsg += "使用臨時文件 '%s' 替代" % conf.resultsFile
                logger.warning(warnMsg)
            except IOError as _:
                errMsg = "無法寫入臨時目錄 ('%s')。" % _
                errMsg += "請確保您的磁盤空間足夠,並且您具有足夠的寫入權限來創建臨時文件和/或目錄"
                #errMsg += "that you have sufficient write permissions to "
                #errMsg += "create temporary files and/or directories"
                raise SqlmapSystemException(errMsg)

        if not found:
            conf.resultsFP.writelines("目標 URL,位置,參數,技術,備註%s" % os.linesep)

        logger.info("在多目標模式下使用 '%s' 作為 CSV 結果文件" % conf.resultsFile)
...
def _createFilesDir():
    ...
    if not os.path.isdir(conf.filePath):
        try:
            ...
            warnMsg = "無法創建文件目錄 '%s'(%s)。使用臨時目錄 '%s' 替代" % (conf.filePath, getUnicode(ex), getUnicode(tempDir))
...
def _createDumpDir():
    ...
    if not os.path.isdir(conf.dumpPath):
        try:
            ...
            warnMsg = "無法創建轉儲目錄 '%s'(%s)。使用臨時目錄 '%s' 替代" % (conf.dumpPath, getUnicode(ex), getUnicode(tempDir))
...
def _createTargetDirs():
    ...
    try:
        ...
        warnMsg = "無法創建輸出目錄 '%s'(%s)。使用臨時目錄 '%s' 替代" % (conf.outputPath, getUnicode(ex), getUnicode(tempDir))
    ...
    try:
        ...
        if "denied" in getUnicode(ex):
            ...
            errMsg = "在嘗試寫入輸出目錄'%s'時出現了問題 "
        errMsg += "(%s)" % (paths.SQLMAP_OUTPUT_PATH, getSafeExString(ex))

        raise SqlmapMissingPrivileges(errMsg)
    except UnicodeError as ex:
        warnMsg = "保存目標數據時出現問題 ('%s')" % getSafeExString(ex)
        logger.warning(warnMsg)
