...
class Dump(object):
    ...
    def _write(self, data, newline=True, console=True, content_type=None):
        ...
        if self._outputFP:
            ...
            try:
                ...
                errMsg = "寫入日誌文件時發生錯誤 ('%s')" % getSafeExString(ex)
    ...
    def setOutputFile(self):
        ...
        try:
            ...
            errMsg = "打開日誌文件時發生錯誤 ('%s')" % getSafeExString(ex)
            raise SqlmapGenericException(errMsg)
    ...
    def currentUser(self, data):
        self.string("當前用戶", data, content_type=CONTENT_TYPE.CURRENT_USER)
    ...
    def currentDb(self, data):
        if Backend.getIdentifiedDbms() in (DBMS.ORACLE, DBMS.PGSQL, DBMS.HSQLDB, DBMS.H2, DBMS.MONETDB, DBMS.VERTICA, DBMS.CRATEDB, DBMS.CACHE, DBMS.FRONTBASE):
            self.string("當前數據庫 (等同於 %s 上的模式)" % Backend.getIdentifiedDbms(), data, content_type=CONTENT_TYPE.CURRENT_DB)
        elif Backend.getIdentifiedDbms() in (DBMS.ALTIBASE, DBMS.DB2, DBMS.MIMERSQL, DBMS.MAXDB, DBMS.VIRTUOSO):
            self.string("當前數據庫 (等同於 %s 上的所有者)" % Backend.getIdentifiedDbms(), data, content_type=CONTENT_TYPE.CURRENT_DB)
        else:
            self.string("當前數據庫", data, content_type=CONTENT_TYPE.CURRENT_DB)

    def hostname(self, data):
        self.string("主機名", data, content_type=CONTENT_TYPE.HOSTNAME)

    def dba(self, data):
        self.string("當前用戶為 DBA", data, content_type=CONTENT_TYPE.IS_DBA)

    def users(self, users):
        self.lister("數據庫管理系統用戶", users, content_type=CONTENT_TYPE.USERS)

    def statements(self, statements):
        self.lister("SQL 語句", statements, content_type=CONTENT_TYPE.STATEMENTS)
    ...
    def userSettings(self, header, userSettings, subHeader, content_type=None):
        ...
        for user in users:
            ...
            if user in self._areAdmins:
                self._write("[*] %s (管理員)%s" % (user, stringSettings))
            else:
    ...
    def dbs(self, dbs):
        self.lister("可用數據庫", dbs, content_type=CONTENT_TYPE.DBS)
    ...
    def dbTables(self, dbTables):
        if isinstance(dbTables, dict) and len(dbTables) > 0:
            ...
            for db, tables in dbTables.items():
                ...
                self._write("數據庫: %s" % unsafeSQLIdentificatorNaming(db) if db and METADB_SUFFIX not in db else "<current>")

                if len(tables) == 1:
                    self._write("[1 張表]")
                else:
                    self._write("[%d 張表]" % len(tables))
            ...
            self.singleString("沒有找到表", content_type=CONTENT_TYPE.TABLES)
        else:
            self.string("表", dbTables, content_type=CONTENT_TYPE.TABLES)
        ...
        if isinstance(tableColumns, dict) and len(tableColumns) > 0:
            ...
            for db, tables in tableColumns.items():
                ...
                for table, columns in tables.items():
                    ...
                    self._write("數據庫: %s\n 表: %s" % (unsafeSQLIdentificatorNaming(db) if db and METADB_SUFFIX not in db else "<current>", unsafeSQLIdentificatorNaming(table)))

                    if len(columns) == 1:
                        self._write("[1 列]")
                    else:
                        self._write("[%d 列]" % len(columns))
                    ...
                    if colType is not None:
                        self._write("| 列名%s | 類型%s |" % (blank1, blank2))
                        self._write("+%s+%s+" % (lines1, lines2))
                    else:
                        self._write("| 列名%s |" % blank1)
                        self._write("+%s+" % lines1)
    ...
    def dbTablesCount(self, dbTables):
        if isinstance(dbTables, dict) and len(dbTables) > 0:
            ...
            for db, counts in dbTables.items():
                ...
                self._write("+%s+%s+" % (lines1, lines2))
                self._write("| 表名%s | 條目數%s |" % (blank1, blank2))
                self._write("+%s+%s+" % (lines1, lines2))
    ...
    def dbTableValues(self, tableValues):
        ...
        try:
            ...
            try:
                ...
                warnMsg = "當前無法使用常規的轉儲目錄。改為使用臨時目錄'%s'" % tempDir
                ...
                try:
                    ...
                    if not os.path.isdir(dumpDbPath):
                        try:
                            ...
                            tempDir = tempfile.mkdtemp(prefix="sqlmapdb")
                            warnMsg = "無法創建轉儲目錄'%s'(%s)。改為使用臨時目錄'%s'" % (dumpDbPath, getSafeExString(ex), tempDir)
        ...
        if count > TRIM_STDOUT_DUMP_SIZE:
            warnMsg = "由於表格大小較大,控制檯輸出將被截斷為最後%d 行" % TRIM_STDOUT_DUMP_SIZE
                    ...
                    if len(value) > MIN_BINARY_DISK_DUMP_SIZE and r'\x' in value:
                        try:
                            ...
                            if any(mimetype.startswith(_) for _ in ("application", "image")):
                                ...
                                warnMsg = "將二進制 ('%s') 內容寫入文件'%s'" % (mimetype, filepath)
                                logger.warning(warnMsg)
        ...
        if conf.dumpFormat == DUMP_FORMAT.SQLITE:
            ...
            logger.info("表格 '%s.%s' 已導出到 SQLITE 數據庫 '%s'" % (db, table, replication.dbpath))
            ...
            msg = "表 '%s.%s' 已導出到 %s 文件 '%s'" % (db, table, conf.dumpFormat, dumpFileName)
            if not warnFile:
                logger.info(msg)
            else:
                logger.warning(msg)
    ...
    def dbColumns(self, dbColumnsDict, colConsider, dbs):
        ...
        for column in dbColumnsDict.keys():
            ...
            if found:
                msg = "在以下數據庫中找到列%s:" % colConsiderStr
    ...
    def registerValue(self, registerData):
        self.string("註冊表鍵值數據", registerData, content_type=CONTENT_TYPE.REG_READ, sort=False)
