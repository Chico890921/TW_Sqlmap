...
def exceptionHandledFunction(threadFunction, silent=False):
    try:
        ...
        if not silent and kb.get("threadContinue") and not kb.get("multipleCtrlC") and not isinstance(ex, (SqlmapUserQuitException, SqlmapSkipTargetException)):
            ...
            logger.error("線程 %s:%s" % (threading.currentThread().getName(), errMsg))
...
def runThreads(numThreads, threadFunction, cleanupFunction=None, forwardException=True, threadChoice=False, startThreadMsg=True):
    ...
    try:
        if threadChoice and conf.threads == numThreads == 1 and not (kb.injection.data and not any(_ not in (PAYLOAD.TECHNIQUE.TIME, PAYLOAD.TECHNIQUE.STACKED) for _ in kb.injection.data)):
            while True:
                message = "請輸入線程數？[按回車鍵使用默認值 %d(當前值)] " % numThreads
                ...
                if choice:
                    ...
                    if isDigit(choice):
                        if int(choice) > MAX_NUMBER_OF_THREADS and not skipThreadCheck:
                            errMsg = "已達到最大線程數%d,以避免可能的連接問題" % MAX_NUMBER_OF_THREADS
            ...
            if numThreads == 1:
                warnMsg = "正在以單線程模式運行。這可能需要一些時間"
                logger.warning(warnMsg)

        if numThreads > 1:
            if startThreadMsg:
                infoMsg = "啟動%d 個線程" % numThreads
                logger.info(infoMsg)
        else:
            try:
                _threadFunction()
        ...
        if kb.lastCtrlCTime and (time.time() - kb.lastCtrlCTime < 1):
            ...
            raise SqlmapUserQuitException("用戶中止（多次按下 Ctrl+C）")

        kb.lastCtrlCTime = time.time()

        if numThreads > 1:
            logger.info("等待線程完成%s" % ("(按下 Ctrl+C)" if isinstance(ex, KeyboardInterrupt) else ""))
        try:
            ...
            kb.multipleCtrlC = True
            raise SqlmapThreadException("用戶中止（多次按下 Ctrl+C）")
        ...
        print()
        kb.threadException = True
        logger.error("線程 %s: '%s'" % (threading.currentThread().getName(), ex))
