...
class Format(object):
    ...
    def getOs(target, info):
        ...
        if info and "type" in info:
            if conf.api:
                infoApi["%s 操作系統" % target] = info
            else:
                infoStr += "%s 操作系統: %s" % (target, Format.humanize(info["type"]))
        ...
        if "technology" in info:
            if conf.api:
                infoApi["Web 應用技術"] = Format.humanize(info["technology"], ", ")
            else:
                infoStr += "\nWeb 應用技術: %s" % Format.humanize(info["technology"], ", ")
...
class Backend(object):
    ...
    def setDbms(dbms):
        ...
        # Little precaution, in theory this condition should always be false
            ...
            msg = "sqlmap 先前將後端 DBMS 識別為%s。" % kb.dbms
            msg += "但現在它被識別為%s。" % dbms
            msg += "請指定應該是哪個 DBMS 正確的[%s(默認)/%s] " % (kb.dbms, dbms)
    ...
    def setVersionList(versionsList):
        if isinstance(versionsList, list):
            ...
            logger.error("versionsList 的格式無效")
    ...
    def setOs(os):
        ...
        # Little precaution, in theory this condition should always be false
            ...
            msg = "sqlmap 先前識別的後端 DBMS 操作系統為 %s。但現在識別為 %s。" % (kb.os, os)
            msg += "請指定哪個操作系統是正確的 [%s(默認)/%s] " % (kb.os, os)
    ...
    def isDbms(dbms):
        if not kb.get("testMode") and all((Backend.getDbms(), Backend.getIdentifiedDbms())) and Backend.getDbms() != Backend.getIdentifiedDbms():
            singleTimeWarnMessage("識別到的數據庫類型（'%s'）與指紋識別到的數據庫類型（'%s'）不一致。如果在枚舉階段遇到問題，請嘗試使用 --flush-session 重新運行" % (Backend.getIdentifiedDbms(), Backend.getDbms()))
            ...
            if condition:
                ...
                if parameter in (conf.base64Parameter or []):
                    try:
                        ...
                        errMsg = "參數'%s'不包含有效的 Base64 編碼值 ('%s')" % (parameter, value)
                ...
                if not conf.multipleTargets and not (conf.csrfToken and re.search(conf.csrfToken, parameter, re.I)):
                    ...
                    if (_.endswith("'") and _.count("'") == 1 or re.search(r'\A9{3,}', _) or re.search(r'\A-\d+\Z', _) or re.search(DUMMY_USER_INJECTION, _)) and not re.search(GOOGLE_ANALYTICS_COOKIE_REGEX, parameter):
                        warnMsg = "您似乎提供了帶有可能是手動 SQL 注入測試的殘留字符/語句的汙染參數值 ('%s')。請始終只使用有效的參數值,以便 sqlmap 能夠正常運行" % element
                        ...
                        message = "您確定要繼續嗎 (sqlmap 可能會遇到問題)？[y/N] "

                        if not readInput(message, default='N', boolean=True):
                            raise SqlmapSilentQuitException
                    elif not _:
                        warnMsg = "參數'%s'的值為空。請始終只使用有效的參數值,以便 sqlmap 能夠正常運行" % parameter
                        #warnMsg += "Please, always use only valid parameter values "
                        #warnMsg += "so sqlmap could be able to run properly"
                        logger.warning(warnMsg)
                        ...
                        if match:
                            try:
                                ...
                                if candidates:
                                    ...
                                    message = "提供的值似乎可以對 %s 參數 '%s' 進行 JSON 反序列化。你想要進行注入嗎？[y/N] " % ("%s " % place if place != parameter else "", parameter)
                            ...
                            #message = "it appears that provided value for %sparameter '%s' " % ("%s " % place if place != parameter else "", parameter)
                            message = "提供的值似乎對 %s 參數 '%s' 有邊界。你想要進行注入嗎？('%s')[y/N] " % ("%s " % place if place != parameter else "", parameter, getUnicode(_))
    ...
    if conf.testParameter:
        if not testableParameters:
            ...
            if len(conf.testParameter) > 1:
                warnMsg = "提供的參數'%s'不在%s 中" % (paramStr, place)
                ...
                if not intersect(USER_AGENT_ALIASES + REFERER_ALIASES + HOST_ALIASES, parameter, True):
                    debugMsg = "提供的參數'%s'不在%s 中" % (paramStr, place)
            ...
            for parameter in conf.testParameter:
                if parameter not in testableParameters:
                    debugMsg = "提供的參數'%s'不在%s 中" % (parameter, place)
                    #debugMsg += "is not inside the %s" % place
                    logger.debug(debugMsg)
                ...
                for encoding in ("hex", "base64"):
                    try:
                        ...
                        if len(decoded) > MIN_ENCODED_LEN_CHECK and all(_ in getBytes(string.printable) for _ in decoded):
                            warnMsg = "提供的參數'%s'似乎是'%s'編碼的" % (parameter, encoding)
...
def getManualDirectories():
    ...
    if conf.webRoot:
        ...
        infoMsg = "將'%s'作為 Web 服務器文檔根目錄" % conf.webRoot
        logger.info(infoMsg)
    elif directories:
        infoMsg = "檢索到 Web 服務器文檔根目錄:'%s'" % directories
        logger.info(infoMsg)
    else:
        warnMsg = "無法自動檢索 Web 服務器文檔根目錄"
        ...
        if choice == '2':
            ...
            infoMsg = "使用生成的目錄列表:%s" % ','.join(directories)
            logger.info(infoMsg)

            msg = "使用任何其他自定義目錄 [按回車鍵為無]: "
...
def getAutoDirectories():
    ...
    if kb.absFilePaths:
        infoMsg = "檢索到 Web 服務器的絕對路徑:'%s'" % ", ".join(ntToPosixSlashes(path) for path in kb.absFilePaths)
...
def dataToTrafficFile(data):
    ...
    try:
        ...
        errMsg = "嘗試寫入流量文件'%s'時出現錯誤 ('%s')" % (conf.trafficFile, getSafeExString(ex))
...
def dataToDumpFile(dumpFile, data):
    try:
        ...
        if "No space left" in getUnicode(ex):
            ...
            errMsg = "寫入轉儲數據到文件時發生錯誤 ('%s')" % getUnicode(ex)
            logger.error(errMsg)
...
def dataToOutFile(filename, data):
    ...
    if data:
        while True:
            ...
            try:
                with open(retVal, "w+b") as f:  # 必須保持為非編碼格式,因為數據是原始的 ASCII 編碼數據
                ...
                if filename != _:
                    ...
                    errMsg = "無法寫入輸出文件 ('%s')" % getSafeExString(ex)
                    #errMsg += "output file ('%s')" % getSafeExString(ex)
                    raise SqlmapGenericException(errMsg)
            except IOError as ex:
                errMsg = "嘗試寫入輸出文件時出現錯誤 ('%s')" % getSafeExString(ex)
...
def checkFile(filename, raiseOnError=True):
    ...
    if not valid and raiseOnError:
        raise SqlmapSystemException("無法讀取文件 '%s'" % filename)
...
def parseTargetDirect():
    ...
    if not details:
        errMsg = "無效的目標詳情,有效的語法示例為'mysql://USER:PASSWORDDBMS_IP:DBMS_PORT/DATABASE_NAME'或'access://DATABASE_FILEPATH'"
        ...
        if dbmsName == conf.dbms or conf.dbms.lower() in data[0]:
            try:
                ...
                if dbmsName in (DBMS.ACCESS, DBMS.SQLITE, DBMS.FIREBIRD):
                    if remote:
                        warnMsg = "不支持直接通過網絡連接%s DBMS" % dbmsName
                    ...
                    errMsg = "缺少遠程連接詳情 (例如'mysql://USER:PASSWORDDBMS_IP:DBMS_PORT/DATABASE_NAME'或'access://DATABASE_FILEPATH')"
                    #errMsg += "'mysql://USER:PASSWORD@DBMS_IP:DBMS_PORT/DATABASE_NAME' "
                    #errMsg += "or 'access://DATABASE_FILEPATH')"
                    raise SqlmapSyntaxException(errMsg)
                ...
                if dbmsName in (DBMS.MSSQL, DBMS.SYBASE):
                    ...
                    if not hasattr(pymssql, "__version__") or pymssql.__version__ < "1.0.2":
                        errMsg = "'%s'第三方庫的版本必須大於等於 1.0.2 才能正常工作。從'%s'下載" % (data[1], data[2])
                ...
                if _sqlalchemy and data[3] and any(_ in _sqlalchemy.dialects.__all__ for _ in (data[3], data[3].split('+')[0])):
                    ...
                    errMsg = "sqlmap 需要'%s'第三方庫才能直接連接到 DBMS'%s'。您可以從'%s'下載它。另一種選擇是安裝支持方言'%s'的'python-sqlalchemy'包" % (data[1], dbmsName, data[2], data[3])
...
def parseTargetUrl():
    ...
    try:
        ...
        errMsg = "給出了無效的 URL '%s'('%s')。請確保在主機名部分沒有任何殘留字符 (例如'['或']')" % (conf.url, getSafeExString(ex))
    ...
    if any((invalid, re.search(r"\s", conf.hostname), '..' in conf.hostname, conf.hostname.startswith('.'), '\n' in originalUrl)):
        errMsg = "無效的目標 URL('%s')" % originalUrl
        raise SqlmapSyntaxException(errMsg)
    ...
    if conf.port < 1 or conf.port > 65535:
        errMsg = "無效的目標 URL 端口 (%d)" % conf.port
        raise SqlmapSyntaxException(errMsg)
    ...
    if (intersect(REFERER_ALIASES, conf.testParameter, True) or conf.level >= 3) and not any(_[0].upper() == HTTP_HEADER.REFERER.upper() for _ in conf.httpHeaders):
        debugMsg = "將 HTTP Referer 頭設置為目標 URL"
        logger.debug(debugMsg)
    ...
    if (intersect(HOST_ALIASES, conf.testParameter, True) or conf.level >= 5) and not any(_[0].upper() == HTTP_HEADER.HOST.upper() for _ in conf.httpHeaders):
        debugMsg = "將 HTTP Host 頭設置為目標 URL"
        logger.debug(debugMsg)
...
def expandAsteriskForColumns(expression):
    ...
    if match:
        infoMsg = "您沒有在查詢中提供字段。sqlmap 將自動檢索列名"
        ...
        if columnsDict and conf.db in columnsDict and conf.tbl in columnsDict[conf.db]:
            ...
            infoMsg = "擴展列名後的查詢為:%s" % expression
...
def getLocalIP():
    ...
    try:
        ...
        debugMsg = "打開與'%s'的套接字連接時出現錯誤" % conf.hostname
...
def getRemoteIP():
    ...
    try:
        ...
        errMsg = "主機名解析問題發生在主機名'%s'" % conf.hostname
    ...
    if isinstance(params, six.string_types):
        ...
        if retVal.count("%s", start, end) == len(params):
            ...
            while True:
                ...
                if match:
                    if count >= len(params):
                        warnMsg = "在字符串格式化過程中參數數量錯誤。請通過電子郵件將內容\"%r | %r | %r\"報告給'%s'" % (format_, params, retVal, DEV_EMAIL_ADDRESS)
...
def parseXmlFile(xmlFile, handler):
    ...
    try:
        ...
        errMsg = "文件'%s'('%s') 似乎有問題。請確保您沒有對其進行任何更改" % (xmlFile, getSafeExString(ex))
...
def getSQLSnippet(dbms, sfile, **variables):
    ...
    if variables:
        errMsg = "在 SQL 文件'%s'中存在未解析的變量%s '%s'" % (sfile, "s" if len(variables) > 1 else "", ", ".join(variables))
        logger.error(errMsg)
        ...
        with kb.locks.cache:
            if filename not in kb.cache.content:
                ...
                try:
                    ...
                    #errMsg = "something went wrong while trying "
                    errMsg = "嘗試讀取文件'%s'的內容時出現錯誤 ('%s')" % (filename, getSafeExString(ex))
                    raise SqlmapSystemException(errMsg)
...
def wasLastResponseDelayed():
    ...
    if deviation and not conf.direct and not conf.disableStats:
        if len(kb.responseTimes[kb.responseTimeMode]) < MIN_TIME_RESPONSES:
            warnMsg = "在具有少於%d 個響應時間的模型上使用了基於時間的標準差方法" % MIN_TIME_RESPONSES
        ...
        if not kb.testMode and retVal:
            if kb.adjustTimeDelay is None:
                msg = "你想讓 sqlmap 嘗試優化 DBMS 延遲響應的值 (選項 '--time-sec') 嗎？[Y/n] "
...
def adjustTimeDelay(lastQueryDuration, lowerStdLimit):
    ...
    if all((_ == candidate for _ in kb.delayCandidates)) and candidate < conf.timeSec:
        if lastQueryDuration / (1.0 * conf.timeSec / candidate) > MIN_VALID_DELAYED_RESPONSE:  # Note: to prevent problems with fast responses for heavy-queries like RANDOMBLOB
            ...
            infoMsg = "由於響應時間良好,將時間延遲調整為%d 秒%s" % (conf.timeSec, 's' if conf.timeSec > 1 else '')
...
def urlencode(value, safe="%&=-_", convall=False, limit=False, spaceplus=False):
    ...
    if value:
        ...
        if Backend.isDbms(DBMS.MSSQL) and not kb.tamperFunctions and any(ord(_) > 255 for _ in value):
            warnMsg = "如果您在使用非 ASCII 標識符名稱時遇到問題,建議您使用'--tamper=charunicodeencode'重新運行"
...
def runningAsAdmin():
    ...
    if PLATFORM in ("posix", "mac"):
        ...
        errMsg = "sqlmap 無法檢查您是否以管理員帳戶身份在此平臺上運行。sqlmap 將假定您是管理員,這對於所請求的接管攻擊正常工作是必需的"
...
def findDynamicContent(firstPage, secondPage):
    ...
    if len(kb.dynamicMarkings) > 0:
        infoMsg = "已標記要移除的動態內容（共 %d 個區域）" % len(kb.dynamicMarkings)
        singleTimeLogMessage(infoMsg)
...
def setOptimize():
    ...
    if not conf.nullConnection:
        debugMsg = "關閉間接使用開關'-o'的開關'--null-connection'"
        logger.debug(debugMsg)
...
def saveConfig(conf, filename):
    ...
    with openFile(filename, "wb") as f:
        try:
            ...
            #errMsg = "something went wrong while trying "
            errMsg = "嘗試寫入配置文件'%s'時出現錯誤 ('%s')" % (filename, getSafeExString(ex))
            raise SqlmapSystemException(errMsg)
        ...
        if data:
            ...
            for key, value in kb.injection.conf.items():
                if value and (not hasattr(conf, key) or (hasattr(conf, key) and not getattr(conf, key))):
                    ...
                    debugMsg = "恢復配置選項'%s'(%s)" % (key, ("'%s'" % value) if isinstance(value, six.string_types) else value)
                    logger.debug(debugMsg)

                    if value and key == "optimize":
                        setOptimize()
        else:
            warnMsg = "對於技術'%s',沒有可用的注入數據" % enumValueToNameLookup(PAYLOAD.TECHNIQUE, technique)
            #warnMsg += "'%s'" % enumValueToNameLookup(PAYLOAD.TECHNIQUE, technique)
            logger.warning(warnMsg)

    except SqlmapDataException:
        errMsg = "舊會話文件中缺少數據。請使用'--flush-session'來處理此錯誤"
...
def showHttpErrorCodes():
    ...
    if kb.httpErrorCodes:
        warnMsg = "運行過程中檢測到的 HTTP 錯誤代碼:\n"
        warnMsg += ", ".join("%d (%s) - %d 次" % (code, _http_client.responses[code] if code in _http_client.responses else '?', count) for code, count in kb.httpErrorCodes.items())
        logger.warning(warnMsg)
        if any((str(_).startswith('4') or str(_).startswith('5')) and _ != _http_client.INTERNAL_SERVER_ERROR and _ != kb.originalCode for _ in kb.httpErrorCodes):
            msg = "過多的 4xx 和/或 5xx HTTP 錯誤代碼可能意味著涉及某種保護機制 (例如 WAF)"
...
def openFile(filename, mode='r', encoding=UNICODE_ENCODING, errors="reversible", buffering=1):  # "buffering=1" means line buffered (Reference: http://stackoverflow.com/a/3168436)
    ...
    if filename == STDIN_PIPE_DASH:
        ...
        try:
            ...
            errMsg = "打開文件'%s'時發生錯誤。請檢查文件的%s 權限,並確保它沒有被其他進程鎖定" % (filename, "寫入" if mode and ('w' in mode or 'a' in mode or '+' in mode) else "讀取")
...
def unhandledExceptionMessage():
    ...
    errMsg = "在%s 中發生未處理的異常。建議您使用官方 GitHub 存儲庫上的最新開發版本重新運行,地址為'%s'。如果異常仍然存在,請在'%s'上打開一個新的問題,提供以下文本和其他重現錯誤所需的信息。開發人員將嘗試重現錯誤,相應修復並與您聯繫\n" % (VERSION_STRING, GIT_PAGE, ISSUES_PAGE)
...
def fetchRandomAgent():
    ...
    if not kb.userAgents:
        debugMsg = "從文件'%s'加載隨機的 HTTP User-Agent 頭" % paths.USER_AGENTS
        ...
        try:
            ...
            errMsg = "無法讀取 HTTP User-Agent 頭文件'%s'" % paths.USER_AGENTS
...
def createGithubIssue(errMsg, excMsg):
    ...
    msg = "\n 你想要自動在官方 Github 倉庫上創建一個新的 (匿名化的) 問題,其中包含未處理的異常信息嗎？[y/N] "
    ...
    if choice:
        ...
        try:
            ...
            if duplicate:
                warnMsg = "問題似乎已經報告過了"
                if closed:
                    warnMsg += "並已解決。請從官方 GitHub 存儲庫的最新開發版本更新" % GIT_PAGE
        ...
        if issueUrl:
            infoMsg = "創建的 Github 問題可以在地址'%s'找到" % issueUrl.group(0)
            logger.info(infoMsg)
            ...
            if regex != payload:
                ...
                if retVal != content:
                    ...
                    if re.search(r"(?i)FRAME[^>]+src=[^>]*%s" % REFLECTED_VALUE_MARKER, retVal):
                        warnMsg = "檢測到包含受攻擊參數值的幀。如果此頁面的攻擊失敗,請確保單獨測試這些值"
...
def expandMnemonics(mnemonics, parser, args):
    ...
    for mnemonic in (mnemonics or "").split(','):
        ...
        if pointer in (None, head):
            ...
            if not options:
                warnMsg = "助記符'%s'無法解析" % name
                logger.warning(warnMsg)
            elif name in options:
                found = name
                debugMsg = "助記符'%s'解析為%s" % (name, found)
                logger.debug(debugMsg)
            else:
                found = sorted(options.keys(), key=len)[0]
                warnMsg = "檢測到歧義 (助記符'%s'可以解析為以下任意一個:%s)。" % (name, ", ".join("'%s'" % key for key in options))
                warnMsg += "解析為最短的那個 ('%s')" % found
                logger.warning(warnMsg)
        ...
        if found:
            ...
            if value is not None:
                ...
                errMsg = "助記符'%s'需要類型為'%s'的值" % (name, found.type)
                raise SqlmapSyntaxException(errMsg)
...
def findPageForms(content, url, raiseException=False, addToTargets=False):
    ...
    try:
        ...
        if re.search(r"(?i)<!DOCTYPE html|<html", content or "") and not re.search(r"(?i)\.js(\?|\Z)", url):
            dbgMsg = "給定 URL('%s') 的 HTML 格式錯誤。將對其進行過濾" % url
            logger.debug(dbgMsg)
    ...
    for form in forms or []:
        try:
            ...
            if conf.crawlExclude and re.search(conf.crawlExclude, form.action or ""):
                dbgMsg = "跳過'%s'" % form.action
                logger.debug(dbgMsg)
                continue

            request = form.click()
        except (ValueError, TypeError) as ex:
            errMsg = "處理頁面表單時出現問題 ('%s')" % getSafeExString(ex)
...
def checkSystemEncoding():
    ...
    if sys.getdefaultencoding() == "cp720":
        try:
            ...
            errMsg = "已知存在與字符集'cp720'相關的 Python 問題 (#1616979)。請訪問'http://blog.oneortheother.info/tip/python-fix-cp720-encoding/index.html'並按照說明進行修復"
...
def evaluateCode(code, variables=None):
    ...
    try:
        ...
        errMsg = "在評估提供的代碼時發生錯誤 ('%s')" % getSafeExString(ex)
        raise SqlmapGenericException(errMsg)
    ...
    def _(value):
        ...
        if value and isinstance(value, six.string_types):
            ...
            if len(value) % 2 != 0:
                ...
                singleTimeWarnMessage("從預期的十六進制格式解碼值 '%s' 時出錯。" % value)
            else:
    ...
    if not conf.loadCookies:
        ...
        try:
            if not cookieJar.filename:
                infoMsg = "從'%s'加載 cookies" % conf.loadCookies
                logger.info(infoMsg)
            ...
            errMsg = "加載 cookies 文件時出現問題 ('%s')" % re.sub(r"(cookies) file '[^']+'", r"\g<1>", getSafeExString(ex))
        ...
        if returncode is not None:
            if not suppress_errors:
                if returncode == 0:
                    ...
                    dataToStdout("子進程被信號 %d 強制終止\n" % returncode)
                elif returncode > 0:
                    dataToStdout("程序意外退出，返回代碼：%d\n" % returncode)
...
def parseRequestFile(reqFile, checkParams=True):
    ...
    def _parseWebScarabLog(content):
        ...
        for request in reqResList:
            ...
            if not method or not url:
                logger.debug("不是有效的 WebScarab 日誌數據")
                continue

            if method.upper() == HTTPMETHOD.POST:
                warnMsg = "不支持從 WebScarab 日誌中的 POST 請求,因為它們的主體內容存儲在單獨的文件中。但是,您可以使用-r 選項逐個加載它們。"
    ...
    if conf.scope:
        logger.info("使用正則表達式 '%s' 進行目標過濾" % conf.scope)
