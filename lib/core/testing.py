...
def vulnTest():
    ...
    TESTS = (
        ("-h", ("查看選項的完整列表 '-hh'",)),
        ("--dependencies", ("sqlmap 需要", "第三方庫")),
        ("-u <url> --data=\"reflect=1\" --flush-session --wizard --disable-coloring", ("請選擇:", "後端 DBMS: SQLite", "當前用戶是 DBA: True", "banner: '3.")),
        ("-u <url> --data=\"code=1\" --code=200 --technique=B --banner --no-cast --flush-session", ("後端 DBMS: SQLite", "banner: '3.", "~COALESCE(CAST(")),
        (u"-c <config> --flush-session --output-dir=\"<tmpdir>\" --smart --roles --statements --hostname --privileges --sql-query=\"SELECT '\u0161u\u0107uraj'\" --technique=U", (u": '\u0161u\u0107uraj'", "在 SQLite 上不可能", "作為輸出目錄")),
        ...
        ("--dummy", ("所有測試的參數似乎都不可注入", "似乎不可注入", "至少沒有一個", "~可能是可注入的")),
        ...
        ("-r <request> --flush-session -v 5 --test-skip=\"heavy\" --save=<config>", ("CloudFlare", "web 應用技術: Express", "可能的 DBMS: 'SQLite'", "User-Agent: foobar", "~類型: 基於時間的盲注", "將命令行選項保存到配置文件")),
        ("-c <config>", ("CloudFlare", "可能的 DBMS: 'SQLite'", "User-Agent: foobar", "~類型: 基於時間的盲注")),
        ...
        ("-u <base> --flush-session --data=\"id=1&_=Eewef6oh\" --chunked --randomize=_ --random-agent --banner", ("獲取隨機的 HTTP User-Agent 標頭值", "參數: id (POST)", "類型: 基於布爾的盲注", "類型: 基於時間的盲注", "類型: UNION 查詢", "banner: '3.")),
        ...
        ("-u <url> --flush-session --encoding=ascii --forms --crawl=2 --threads=2 --banner", ("總共有 2 個目標", "可能是可注入的", "類型: UNION 查詢", "banner: '3.")),
        ("-u <base> --flush-session --technique=BU --data=\"{\\\"id\\\": 1}\" --banner", ("可能是可注入的", "3 列", "Payload: {\"id\"", "類型: 基於布爾的盲注", "類型: UNION 查詢", "banner: '3.")),
        ("-u <base> --flush-session -H \"Foo: Bar\" -H \"Sna: Fu\" --data=\"<root><param name=\\\"id\\\" value=\\\"1*\\\"/></root>\" --union-char=1 --mobile --answers=\"smartphone=3\" --banner --smart -v 5", ("可能是可注入的", "Payload: <root><param name=\"id\" value=\"1", "類型: 基於布爾的盲注", "類型: 基於時間的盲注", "類型: UNION 查詢", "banner: '3.", "Nexus", "Sna: Fu", "Foo: Bar")),
        ("-u <base> --flush-session --technique=BU --method=PUT --data=\"a=1;id=1;b=2\" --param-del=\";\" --skip-static --har=<tmpfile> --dump -T users --start=1 --stop=2", ("可能是可注入的", "參數: id (PUT)", "類型: 基於布爾的盲注", "類型: UNION 查詢", "2 個條目")),
        ("-u <url> --flush-session -H \"id: 1*\" --tables -t <tmpfile>", ("可能是可注入的", "參數: id #1* ((custom) HEADER)", "類型: 基於布爾的盲注", "類型: 基於時間的盲注", "類型: UNION 查詢", " users ")),
        ...
        ("-u <url> --flush-session --cookie=\"PHPSESSID=d41d8cd98f00b204e9800998ecf8427e; id=1*; id2=2\" --tables --union-cols=3", ("可能是可注入的", "Cookie #1* ((custom) HEADER)", "類型: 基於布爾的盲注", "類型: 基於時間的盲注", "類型: UNION 查詢", " users ")),
        ...
        ("-u <url> --flush-session --parse-errors --test-filter=\"subquery\" --eval=\"import hashlib; id2=2; id3=hashlib.md5(id.encode()).hexdigest()\" --referer=\"localhost\"", ("可能是可注入的", ": 語法錯誤", "後端 DBMS: SQLite", "WHERE or HAVING clause (subquery")),
        ...
        ("-u <url> --technique=U --fresh-queries --force-partial --dump -T users --dump-format=HTML --answers=\"crack=n\" -v 3", ("執行了 6 個查詢", "nameisnull", "~使用默認字典", "轉儲到 HTML 文件")),
        ("-u <url> --flush-session --technique=BU --all", ("5 個條目", "類型: 基於布爾的盲注", "類型: UNION 查詢", "luther", "blisset", "fluffy", "179ad45c6ce2cb97cf1029e212046e81", "NULL", "nameisnull", "testpass")),
        ("-u <url> -z \"tec=B\" --hex --fresh-queries --threads=4 --sql-query=\"SELECT * FROM users\"", ("SELECT * FROM users [5]", "nameisnull")),
        ("-u \"<url>&echo=foobar*\" --flush-session", ("可能存在跨站腳本攻擊漏洞",)),
        ("-u \"<url>&query=*\" --flush-session --technique=Q --banner", ("標題: SQLite 內聯查詢", "banner: '3.")),
        ("-d \"<direct>\" --flush-session --dump -T users --dump-format=SQLITE --binary-fields=name --where \"id=3\"", ("7775", "179ad45c6ce2cb97cf1029e212046e81 (testpass)", "轉儲到 SQLITE 數據庫")),
    ...
    if not vulnserver._alive:
        logger.error("在 vulnserver 實例化過程中出現問題 (地址:'http://%s:%s')" % (address, port))
        return False
    else:
        logger.info("vulnserver 在 'http://%s:%s' 上運行..." % (address, port))
    ...
    clearConsoleLine()
    if retVal:
        logger.info("漏洞測試最終結果:通過")
    else:
        logger.error("漏洞測試最終結果:失敗")
...
def smokeTest():
                ...
                if isinstance(candidate, str):
                    if '\\' in candidate:
                        try:
                            ...
                            errMsg = "煙霧測試在編譯'%s'時失敗" % candidate
                            logger.error(errMsg)
                            raise
                else:
                    _(candidate)
    ...
    clearConsoleLine()
    if retVal:
        logger.info("煙霧測試最終結果:通過")
    else:
        logger.error("煙霧測試最終結果:失敗")
