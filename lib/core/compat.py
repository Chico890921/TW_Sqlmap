...
class WichmannHill(random.Random):
    ...
    def setstate(self, state):
        ...
        if version == 1:
            ...
            raise ValueError("傳入的狀態版本為 %s，"
                             "但 Random.setstate() 當前使用的是版本 %s" %
    ...
    def __whseed(self, x=0, y=0, z=0):
        ...
        if not type(x) == type(y) == type(z) == int:
            raise TypeError('種子值必須是整數')
        if not (0 <= x < 256 and 0 <= y < 256 and 0 <= z < 256):
            raise ValueError('種子值必須在 0 到 255 之間')
        if 0 == x == y == z:
...
def cmp_to_key(mycmp):
    ...
    class K(object):
        ...
        def __hash__(self):
            raise TypeError('未實現 __hash__ 方法')
