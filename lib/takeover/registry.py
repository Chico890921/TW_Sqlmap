...
class Registry(object):
    ...
    def _createRemoteBatchFile(self):
        logger.debug("正在創建批處理文件 '%s'" % self._batPathRemote)
    ...
    def readRegKey(self, regKey, regValue, parse=False):
        ...
        logger.debug("讀取註冊表鍵 '%s' 的值 '%s'" % (regKey, regValue))
    ...
    def addRegKey(self, regKey, regValue, regType, regData):
        ...
        debugMsg = "將註冊表鍵值 '%s' 添加到註冊表鍵 '%s'" % (self._regValue, self._regKey)
    ...
    def delRegKey(self, regKey, regValue):
        ...
        debugMsg = "從註冊表鍵 '%s' 中刪除註冊表鍵值 '%s'" % (self._regKey, self._regValue)
