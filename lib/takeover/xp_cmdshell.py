...
class XP_cmdshell(object):
    ...
    def _xpCmdshellConfigure2005(self, mode):
        debugMsg = "使用 sp_configure 存儲過程配置 xp_cmdshell"
    ...
    def _xpCmdshellConfigure2000(self, mode):
        debugMsg = "使用 sp_addextendedproc 存儲過程配置 xp_cmdshell"
        #debugMsg += "stored procedure"
        logger.debug(debugMsg)
    ...
    def _xpCmdshellTest(self):
        ...
        logger.info("測試 xp_cmdshell 擴展過程是否可用")
        output = self.xpCmdshellEvalCmd("echo 1")

        if output == "1":
            logger.info("Xp_cmdShell 擴展過程可用")
        elif isNoneValue(output) and conf.dbmsCred:
            errMsg = "似乎用於在後端文件系統中存儲控制檯輸出的臨時目錄 ('%s') 對於 DBMS 進程沒有寫入權限。建議您使用'--tmp-path'選項手動調整它,否則您將無法檢索到命令的輸出" % self.getRemoteTempPath()
            ...
            logger.error("無法檢索 xp_cmdShell 輸出")
        else:
            logger.info("Xp_cmdShell 擴展過程可用")
    ...
    def xpCmdshellInit(self):
        if not kb.xpCmdshellAvailable:
            infoMsg = "正在檢查 xp_cmdshell 擴展過程是否可用,請稍候..."
            ...
            if result:
                ...
                message = "似乎 xp_cmdshell 擴展過程不可用。您是否希望 sqlmap 嘗試重新啟用它？[Y/n] "
                ...
                if readInput(message, default='Y', boolean=True):
                    ...
                    if self._xpCmdshellCheck():
                        logger.info("xp_cmdshell 已成功重新啟用")
                        kb.xpCmdshellAvailable = True

                    else:
                        logger.warning("xp_cmdshell 重新啟用失敗")

                        logger.info("使用 sp_OACreate 創建 xp_cmdshell")
                        self._xpCmdshellConfigure(0)
                        self._xpCmdshellCreate()

                        if self._xpCmdshellCheck():
                            logger.info("xp_cmdshell 創建成功")
                            kb.xpCmdshellAvailable = True

                        else:
                            warnMsg = "xp_cmdshell 創建失敗,可能是因為 sp_OACreate 被禁用"
            ...
            if not kb.xpCmdshellAvailable:
                errMsg = "無法在沒有 xp_cmdshell 的情況下繼續進行"
                raise SqlmapUnsupportedFeatureException(errMsg)

        debugMsg = "創建一個支持表來寫入命令的標準輸出"
        #debugMsg += "output to"
        logger.debug(debugMsg)
