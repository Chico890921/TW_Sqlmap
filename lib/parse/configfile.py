...
def configFileProxy(section, option, datatype):
    ...
    if config.has_option(section, option):
        try:
            ...
            errMsg = "處理提供的配置文件中的選項'%s'時發生錯誤 ('%s')" % (option, getUnicode(ex))
        ...
        debugMsg = "配置文件中缺少請求的選項'%s'(部分'%s'),忽略該選項。跳過到下一個選項。" % (option, section)
...
def configFileParser(configFile):
    ...
    try:
        ...
        errMsg = "您提供了一個無效和/或不可讀的配置文件 ('%s')" % getSafeExString(ex)
        raise SqlmapSyntaxException(errMsg)
