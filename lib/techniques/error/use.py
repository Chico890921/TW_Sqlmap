...
def _oneShotErrorUse(expression, field=None, chunkTest=False):
    ...
    if retVal and PARTIAL_VALUE_MARKER in retVal:
        ...
        logger.info("恢復部分值:'%s'" % _formatPartialContent(partialValue))
        ...
        try:
            while True:
                ...
                if output is not None:
                    ...
                    if trimmed:
                        if not chunkTest:
                            warnMsg = "檢測到可能由於長度和/或內容而被服務器截斷的輸出:"
...
def errorUse(expression, dump=False):
        ...
        if limitCond:
            ...
            if isNumPosStrValue(count):
                if isinstance(stopLimit, int) and stopLimit > 0:
                    ...
                    debugMsg = "使用的 SQL 查詢返回%d 個%s" % (stopLimit, "條目" if stopLimit > 1 else "條目")
            ...
            if isNumPosStrValue(count) and int(count) > 1:
                if " ORDER BY " in expression and (stopLimit - startLimit) > SLOW_ORDER_COUNT_THRESHOLD:
                    message = "由於表的大小巨大,你想要移除 ORDER BY 子句以獲得速度而犧牲一致性嗎？[y/N] "
                ...
                try:
                    ...
                    errMsg = "邊界限制 (%d,%d) 太大。請使用'--fresh-queries'開關重新運行" % (startLimit, stopLimit)
                ...
                if kb.dumpTable and (len(expressionFieldsList) < (stopLimit - startLimit) > CHECK_ZERO_COLUMNS_THRESHOLD):
                    for field in expressionFieldsList:
                        if _oneShotErrorUse("SELECT COUNT(%s) FROM %s" % (field, kb.dumpTable)) == '0':
                            ...
                            debugMsg = "表格'%s'的列'%s'不會被轉儲,因為它似乎是空的" % (kb.dumpTable, field)
                            #debugMsg += "dumped as it appears to be empty"
                            logger.debug(debugMsg)

                if stopLimit > TURN_OFF_RESUME_INFO_LIMIT:
                    kb.suppressResumeInfo = True
                    debugMsg = "由於行數過多,抑制可能的恢復控制檯信息。這可能需要很長時間"
    ...
    if not kb.bruteMode:
        debugMsg = "在%.2f 秒內執行了%d 個查詢" % (duration, kb.counters[getTechnique()]) + ("y" if kb.counters[getTechnique()] == 1 else "ies")
        logger.debug(debugMsg)
