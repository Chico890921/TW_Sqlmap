...
def _oneShotUnionUse(expression, unpack=True, limited=False):
    ...
    if retVal is None:
        ...
        if page and kb.chars.start.upper() in page and kb.chars.start not in page:
            singleTimeWarnMessage("結果似乎被強制轉換為大寫，sqlmap 將自動轉換為小寫")
        ...
        if retVal is not None:
            ...
            if trimmed:
                warnMsg = "檢測到可能被服務器截斷的輸出 (可能是由於其長度和/或內容):"
                #warnMsg += "(probably due to its length and/or content): "
                warnMsg += safecharencode(trimmed)
                logger.warning(warnMsg)

            elif re.search(r"ORDER BY [^ ]+\Z", expression):
                debugMsg = "重新嘗試失敗的 SQL 查詢,不包括 ORDER BY 子句"
                ...
                debugMsg = "關閉 NATIONAL CHARACTER 轉換"  # NOTE: in some cases there are "known" incompatibilities between original columns and NCHAR (e.g. http://testphp.vulnweb.com/artists.php?artist=1)
                singleTimeDebugMessage(debugMsg)
...
def configUnion(char=None, columns=None):
    ...
    def _configUnionCols(columns):
        ...
        if not isDigit(colsStart) or not isDigit(colsStop):
            raise SqlmapSyntaxException("--union-cols 參數必須是一個整數範圍")

        conf.uColsStart, conf.uColsStop = int(colsStart), int(colsStop)

        if conf.uColsStart > conf.uColsStop:
            errMsg = "--union-cols 範圍必須表示列的較低到較高的數量"
...
def unionUse(expression, unpack=True, dump=False):
    ...
    if expressionFieldsList and len(expressionFieldsList) > 1 and "ORDER BY" in expression.upper():
        ...
        debugMsg = "從語句中刪除 ORDER BY 子句,因為它與 UNION 查詢的 SQL 注入不兼容"
        ...
        if limitCond:
            ...
            if isNumPosStrValue(count):
                if isinstance(stopLimit, int) and stopLimit > 0:
                    ...
                    debugMsg = "使用的 SQL 查詢返回 %d %s" % (stopLimit, "條記錄" if stopLimit > 1 else "條記錄")
                ...
                warnMsg = "無法計算所提供的 SQL 查詢的條目數。sqlmap 將假設它只返回一個條目"
            ...
            if isNumPosStrValue(count) and int(count) > 1:
                ...
                try:
                    ...
                    errMsg = "邊界限制 (%d,%d) 太大。請重新運行,並使用'--fresh-queries'開關" % (startLimit, stopLimit)
                ...
                if stopLimit > TURN_OFF_RESUME_INFO_LIMIT:
                    ...
                    debugMsg = "由於可能需要很長時間,因此不顯示大量行的恢復控制檯信息"
                ...
                try:
                    ...
                    warnMsg = "用戶在枚舉過程中中止。sqlmap 將顯示部分輸出"
    ...
    if not kb.bruteMode:
        debugMsg = "在 %.2f 秒內執行了 %d 個查詢" % (duration, kb.counters[PAYLOAD.TECHNIQUE.UNION]), 'y' if kb.counters[PAYLOAD.TECHNIQUE.UNION] == 1 else "ies"
        logger.debug(debugMsg)
