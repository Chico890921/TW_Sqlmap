...
def bisection(payload, expression, length=None, charsetType=None, firstChar=None, lastChar=None, dump=False):
    ...
    if retVal:
        if conf.repair and INFERENCE_UNKNOWN_CHAR in retVal:
            ...
            if retVal and conf.hexConvert:
                partialValue = retVal
                infoMsg = "恢復部分值:%s" % safecharencode(partialValue)
                logger.info(infoMsg)
            ...
            infoMsg = "已恢復:%s" % safecharencode(retVal)
            logger.info(infoMsg)
    ...
    try:
        ...
        if numThreads > 1:
            if not timeBasedCompare or kb.forceThreads:
                debugMsg = "啟動%d 個線程" % numThreads
                logger.debug(debugMsg)
            else:
                numThreads = 1

        if conf.threads == 1 and not any((timeBasedCompare, conf.predictOutput)):
            warnMsg = "以單線程模式運行。請考慮使用'--threads'選項以加快數據檢索速度"
        ...
        if conf.verbose in (1, 2) and not any((showEta, conf.api, kb.bruteMode)):
            if isinstance(length, int) and numThreads > 1:
                dataToStdout("[%s] [INFO] 已檢索: %s" % (time.strftime("%X"), "_" * min(length, conf.progressWidth)))
                dataToStdout("\r[%s] [INFO] 已檢索: " % time.strftime("%X"))
            else:
                dataToStdout("\r[%s] [INFO] 已檢索: " % time.strftime("%X"))
        ...
        def validateChar(idx, value):
            ...
            if result and timeBasedCompare and getTechniqueData().trueCode:
                ...
                if not result:
                    warnMsg = "在驗證階段檢測到的 HTTP 狀態碼'%s'與預期的狀態碼'%s'不一致" % (threadData.lastCode, getTechniqueData().trueCode)
                    singleTimeWarnMessage(warnMsg)
        ...
        def getChar(idx, charTbl=None, continuousOrder=True, expand=charsetType is None, shiftTable=None, retried=None):
            ...
            if continuousOrder:
                    ...
                    if not timeBasedCompare and getTechniqueData() is not None:
                        ...
                        if unexpectedCode:
                            if threadData.lastCode is not None:
                                warnMsg = "檢測到意外的 HTTP 狀態碼'%s'。" % threadData.lastCode
                            else:
                                warnMsg = "檢測到意外的響應。"

                            warnMsg += "在類似情況下將使用 (額外的) 驗證步驟"
                                    ...
                                    if (retried or 0) < MAX_REVALIDATION_STEPS:
                                        ...
                                        if timeBasedCompare:
                                            if kb.adjustTimeDelay is not ADJUST_TIME_DELAY.DISABLE:
                                                conf.timeSec += 1
                                                warnMsg = "增加時間延遲到%d 秒" % conf.timeSec + ("s" if conf.timeSec > 1 else "")
                                        ...
                                        errMsg = "無法正確驗證最後一個字符值 ('%s').." % decodeIntToUnicode(retVal)
                                        logger.error(errMsg)
                ...
                if "'%s'" % CHAR_INFERENCE_MARK in payload and conf.charset:
                    errMsg = "在'%s'上不支持'--charset'選項" % Backend.getIdentifiedDbms()
        ...
        if numThreads > 1 and isinstance(length, int) and length > 1:
                        ...
                        if kb.threadContinue:
                            if showEta:
                                ...
                                if conf.verbose in (1, 2) and not any((showEta, conf.api, kb.bruteMode)):
                                    ...
                                    dataToStdout("\r[%s] [信息] 已獲取: %s" % (time.strftime("%X"), output))
            ...
            if None in value:
                ...
                if partialValue:
                    infoMsg = "\r[%s] [信息] 部分檢索:%s" % (time.strftime("%X"), filterControlChars(partialValue))
            else:
                finalValue = "".join(value)
                infoMsg = "\r[%s] [信息] 檢索到:%s" % (time.strftime("%X"), filterControlChars(finalValue))
    ...
    if conf.hexConvert and not any((abortedFlag, conf.api, kb.bruteMode)):
        ...
        if (conf.verbose in (1, 2) and showEta) or conf.verbose >= 3:
            infoMsg = "檢索到:%s" % filterControlChars(finalValue)
            logger.info(infoMsg)

    if kb.threadException:
        raise SqlmapThreadException("線程內部發生了意外情況")
...
def queryOutputLength(expression, payload):
    ...
    debugMsg = "在%.2f 秒內執行了%d 個查詢" % (calculateDeltaSeconds(start), count) + ("y" if count == 1 else "ies")
    logger.debug(debugMsg)
