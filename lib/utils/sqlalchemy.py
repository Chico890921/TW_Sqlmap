...
class SQLAlchemy(GenericConnector):
            ...
            try:
                if not self.port and self.db:
                    if not os.path.exists(self.db):
                        raise SqlmapFilePathException("提供的數據庫文件'%s'不存在" % self.db)
                ...
                if "_get_server_version_info" in traceback.format_exc():
                    try:
                        ...
                        if int(pymssql.__version__[0]) < 2:
                            raise SqlmapConnectionException("SQLAlchemy 連接問題（過時版本的 pymssql（'%s'）導致問題）" % pymssql.__version__)
                    except ImportError:
                        pass
                elif "invalid literal for int() with base 10: '0b" in traceback.format_exc():
                    raise SqlmapConnectionException("SQLAlchemy 連接問題 ('https://bitbucket.org/zzzeek/sqlalchemy/issues/3975')")
                else:
                    pass
            except SqlmapFilePathException:
                raise
            except Exception as ex:
                raise SqlmapConnectionException("SQLAlchemy 連接問題（'%s'）" % getSafeExString(ex))

            self.printConnected()
        else:
            raise SqlmapMissingDependence("未檢測到 SQLAlchemy 庫（例如，使用命令 'pip%s install SQLAlchemy' 安裝）" % ('3' if six.PY3 else ""))
    ...
    def fetchall(self):
        try:
            ...
            logger.log(logging.WARN if conf.dbmsHandler else logging.DEBUG, "(遠程) %s" % getSafeExString(ex))
            return None
