    ...
    def _get_cursor(self):
        ...
        if threadData.hashDBCursor is None:
            try:
                ...
                errMsg = "打開會話文件 '%s' 時發生錯誤 ('%s')" % (self.filepath, getSafeExString(ex))
    ...
    def retrieve(self, key, unserialize=False):
                ...
                for _ in xrange(HASHDB_RETRIEVE_RETRIES):
                    try:
                        ...
                        if any(_ in getSafeExString(ex) for _ in ("locked", "no such table")):
                            warnMsg = "訪問會話文件 '%s' 時發生問題 ('%s')" % (self.filepath, getSafeExString(ex))
                            singleTimeWarnMessage(warnMsg)
                        elif "Could not decode" in getSafeExString(ex):
                            break
                        else:
                            errMsg = "訪問會話文件 '%s' 時發生錯誤 ('%s')。" % (self.filepath, getSafeExString(ex))
                            errMsg += "如果問題仍然存在,請使用 '--flush-session' 重新運行"
                            raise SqlmapConnectionException(errMsg)
                    else:
                        break
        ...
        if retVal and unserialize:
            try:
                ...
                retVal = None
                warnMsg = "反序列化會話鍵 '%s' 的值時發生錯誤。如果問題仍然存在,請使用 '--flush-session' 重新運行"
                ...
                while True:
                    try:
                        ...
                        if not os.path.exists(self.filepath):
                            debugMsg = "會話文件 '%s' 不存在" % self.filepath
                            logger.debug(debugMsg)
                            break

                        if retries == 0:
                            #warnMsg = "there has been a problem while writing to "
                            warnMsg = "寫入會話文件時出現問題 ('%s')" % getSafeExString(ex)
                            logger.warning(warnMsg)
