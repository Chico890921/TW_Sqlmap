...
def pivotDumpTable(table, colList, count=None, blind=True, alias=None):
    ...
    if conf.pivotColumn:
        for _ in colList:
            if re.search(r"(.+\.)?%s" % re.escape(conf.pivotColumn), _, re.I):
                infoMsg = "使用列 '%s' 作為數據行的樞軸" % conf.pivotColumn
    ...
    if not validPivotValue:
        for column in colList:
            infoMsg = "獲取列 '%s' 的不同值數量" % column.replace(("%s." % alias) if alias else "", "")
            ...
            if isNumPosStrValue(value):
                ...
                if value == count:
                    infoMsg = "使用列 '%s' 作為數據行的樞軸" % column.replace(("%s." % alias) if alias else "")
                    #infoMsg += "for retrieving row data"
                    logger.info(infoMsg)
        ...
        if not validPivotValue:
            warnMsg = "未提供適當的樞軸列 (具有唯一值)。無法檢索所有行"
    ...
    try:
            ...
            for column in colList:
                ...
                if conf.limitStart or conf.limitStop:
                    if conf.limitStart and (i + 1) < conf.limitStart:
                        warnMsg = "跳過前 %d 個樞軸點值" % conf.limitStart
        ...
        errMsg = "檢測到連接異常 ('%s')。sqlmap 將顯示部分輸出" % getSafeExString(ex)
