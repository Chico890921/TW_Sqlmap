...
def storeHashesToFile(attack_dict):
    ...
    if kb.choices.storeHashes is None:
        message = "您是否要將哈希值存儲到臨時文件中,以便以後使用其他工具進行進一步處理 [y/N] "
...
def attackDumpedTable():
    if kb.data.dumpedTable:
        ...
        if binary_fields:
            ...
            warnMsg = "檢測到潛在的二進制字段 ('%s')。如果遇到任何問題,建議您使用 '--fresh-queries --binary-fields=\"%s\"' 重新運行錶轉儲" % (_, _)
        ...
        if attack_dict:
            infoMsg = "在%s 列中識別到可能的密碼哈希值" % ("多個" if len(col_passwords) > 1 else "列")
            infoMsg += "'%s'" % ", ".join(col for col in col_passwords)
            logger.info(infoMsg)

            storeHashesToFile(attack_dict)

            message = "您是否要通過基於字典的攻擊來破解它們？%s" % ("[y/N/q]" if conf.multipleTargets else "[Y/n/q]")
    ...
    try:
        for word in wordlist:
            ...
            try:
                ...
                if current in hashes:
                    for item in attack_info[:]:
                        ...
                        if hash_ == current:
                            ...
                            infoMsg = "\r[%s] [信息] 破解密碼 '%s'" % (time.strftime("%X"), word)
                    ...
                    status = "當前狀態: %s... %s" % (word.ljust(5)[:5], ROTATING_CHARS[rotator])
                ...
                warnMsg = "在哈希條目時出現問題:%s('%s')。" % (repr(word), getSafeExString(ex))
                warnMsg += "請通過電子郵件報告給 '%s'" % DEV_EMAIL_ADDRESS
                    ...
                    try:
                        ...
                        if item and hash_ not in keys:
                            ...
                            if not resumed:
                                ...
                                infoMsg = "恢復密碼 '%s' 用於哈希值 '%s'" % (resumed, hash_)
                                if user and not user.startswith(DUMMY_USER_PREFIX):
                                    infoMsg += ",對於用戶 '%s'" % user
                                logger.info(infoMsg)
        ...
        if not kb.wordlists:
            while not kb.wordlists:
                ...
                try:
                    ...
                    warnMsg = "加載字典時出現問題 ('%s')" % getSafeExString(ex)
                    #warnMsg += " ('%s')" % getSafeExString(ex)
                    logger.critical(warnMsg)

            message = "您是否要使用常見密碼後綴？(速度較慢！)[y/N] "

            if readInput(message, default='N', boolean=True):
                suffix_list += COMMON_PASSWORD_SUFFIXES

        infoMsg = "開始基於字典的破解 (%s)" % __functions__[hash_regex].__name__
        logger.info(infoMsg)
                ...
                try:
                    if _multiprocessing:
                        if _multiprocessing.cpu_count() > 1:
                            infoMsg = "啟動 %d 個進程" % _multiprocessing.cpu_count()
                        ...
                        warnMsg = "在此平臺上,多進程哈希破解當前%s" % ("不受支持" if not conf.disableMulti else "已禁用")
                    ...
                    warnMsg = "用戶在基於字典的攻擊階段中中止 (按下了 Ctrl+C)"
                    logger.warning(warnMsg)
