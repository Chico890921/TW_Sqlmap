...
def _search(dork):
    ...
    try:
        ...
        errMsg = "無法連接到 Google('%s')" % getSafeExString(ex)
        raise SqlmapConnectionException(errMsg)

    gpage = conf.googlePage if conf.googlePage > 1 else 1
    logger.info("使用搜索結果頁面 #%d" % gpage)
    ...
    try:
        ...
        try:
            ...
            warnMsg = "嘗試獲取錯誤頁面信息時發生問題 (%s)" % getSafeExString(_)
    ...
    if not retVal and "detected unusual traffic" in page:
        warnMsg = "Google 檢測到來自使用的 IP 地址的 '異常' 流量,禁止進一步搜索"
    ...
    if not retVal:
        message = "未找到可用的鏈接。你想要做什麼？"
        message += "\n[1] 使用 DuckDuckGo (默認) 進行重試"
        message += "\n[2] 使用 Bing 進行重試"
        message += "\n[3] 退出"
        ...
        try:
            ...
            try:
                ...
                warnMsg = "嘗試獲取錯誤頁面信息時連接超時 (%d)" % ex.code
        ...
        if not retVal and "issue with the Tor Exit Node you are currently using" in page:
            warnMsg = "DuckDuckGo 檢測到來自使用的 (Tor)IP 地址的 '異常' 流量"
