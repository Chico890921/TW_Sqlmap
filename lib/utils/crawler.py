...
def crawl(target, post=None, cookie=None):
    ...
    try:
        ...
        def crawlThread():
            ...
            while kb.threadContinue:
                ...
                try:
                    ...
                    errMsg = "檢測到連接異常 ('%s')。跳過 URL '%s'" % (getSafeExString(ex), current)
                    #errMsg += "URL '%s'" % current
                    logger.critical(errMsg)
                except SqlmapSyntaxException:
                    errMsg = "檢測到無效的 URL。跳過 '%s'" % current
                    logger.critical(errMsg)
                except _http_client.InvalidURL as ex:
                    errMsg = "檢測到無效的 URL('%s')。跳過 URL '%s'" % (getSafeExString(ex), current)
                    #errMsg += "URL '%s'" % current
                    logger.critical(errMsg)
                ...
                if conf.verbose in (1, 2):
                    ...
                    status = '已訪問 %d/%d 個鏈接 (%d%%)' % (threadData.shared.count, threadData.shared.length, round(100.0 * threadData.shared.count / threadData.shared.length))
        ...
        if kb.checkSitemap is None:
            message = "你想要檢查站點是否存在網站地圖 (.xml) 嗎？[y/N] "
        ...
        if kb.checkSitemap:
            ...
            try:
                ...
                if found:
                    ...
                    logger.info("%s 個鏈接已找到" % ("沒有" if not items else len(items)))

        if not conf.bulkFile:
            infoMsg = "啟動目標 URL '%s' 的爬蟲" % target
            logger.info(infoMsg)
        ...
        warnMsg = "用戶在爬取過程中中止。sqlmap 將使用部分列表"
...
def storeResultsToFile(results):
    ...
    if kb.storeCrawlingChoice is None:
        message = "您是否要將爬取結果存儲到臨時文件中,以便以後使用其他工具進行進一步處理 [y/N] "
    ...
    if kb.storeCrawlingChoice:
        ...
        infoMsg = "將爬取結果寫入臨時文件 '%s'" % filename
        logger.info(infoMsg)
