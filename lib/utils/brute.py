...
def tableExists(tableFile, regex=None):
    if kb.choices.tableExists is None and not any(_ for _ in kb.injection.data if _ not in (PAYLOAD.TECHNIQUE.TIME, PAYLOAD.TECHNIQUE.STACKED)) and not conf.direct:
        warnMsg = "不建議在常規列存在檢查中使用 '%s' 和/或 '%s'" % (PAYLOAD.SQLINJECTION[PAYLOAD.TECHNIQUE.TIME], PAYLOAD.SQLINJECTION[PAYLOAD.TECHNIQUE.STACKED])
    ...
    if result:
        errMsg = "由於檢測到無效結果,無法使用表存在檢查 "
        errMsg += "(很可能是由於所使用的注入無法區分錯誤結果)"
    ...
    message = "您想要使用哪個常見表 (單詞列表) 文件？\n"
    message += "[1] 默認文件 '%s'(按回車鍵)\n" % tableFile
    message += "[2] 自定義文件"
    choice = readInput(message, default='1')

    if choice == '2':
        message = "請提供自定義常見表文件的位置:\n"
        tableFile = readInput(message) or tableFile

    infoMsg = "使用 '%s' 中的項執行表存在檢查" % tableFile
    logger.info(infoMsg)
    ...
    for conf.db in (conf.db.split(',') if conf.db else [conf.db]):
        ...
        def tableExistsThread():
            ...
            while kb.threadContinue:
                ...
                if conf.verbose in (1, 2):
                    status = '%d/%d 項 (%d%%)' % (threadData.shared.count, threadData.shared.limit, round(100.0 * threadData.shared.count / threadData.shared.limit))
                    dataToStdout("\r[%s] [信息] 嘗試了 %s" % (time.strftime("%X"), status), True)
        ...
        try:
            ...
            warnMsg = "用戶在表存在檢查過程中中止。sqlmap 將顯示部分輸出"
...
def columnExists(columnFile, regex=None):
    ...
    if result:
        errMsg = "由於檢測到無效結果,無法使用列存在檢查 "
        errMsg += "(很可能是由於所使用的注入無法區分錯誤結果)"
        #errMsg += "to distinguish erroneous results)"
        raise SqlmapDataException(errMsg)

    message = "您想要使用哪個常見列 (單詞列表) 文件？\n"
    message += "[1] 默認文件 '%s'(按回車鍵)\n" % columnFile
    message += "[2] 自定義文件"
    choice = readInput(message, default='1')

    if choice == '2':
        message = "請提供自定義常見列文件的位置:\n"
        columnFile = readInput(message) or columnFile

    infoMsg = "使用 '%s' 中的項執行列存在檢查" % columnFile
    logger.info(infoMsg)
    ...
    for table in conf.tbl.split(','):
        ...
        try:
            ...
            warnMsg = "用戶在列存在檢查過程中中止。sqlmap 將顯示部分輸出"
            #warnMsg += "check. sqlmap will display partial output"
            logger.warning(warnMsg)
        finally:
            kb.bruteMode = False
...
def fileExists(pathFile):
    ...
    message = "您想要使用哪個常見文件文件？\n"
    message += "[1] 默認文件 '%s'(按回車鍵)\n" % pathFile
    message += "[2] 自定義文件"
    choice = readInput(message, default='1')

    if choice == '2':
        message = "請提供自定義常見文件文件的位置:\n"
        pathFile = readInput(message) or pathFile

    infoMsg = "使用 '%s' 中的項執行文件存在檢查" % pathFile
    logger.info(infoMsg)
    ...
    try:
        ...
        warnMsg = "用戶在文件存在檢查過程中中止。sqlmap 將顯示部分輸出"
        #warnMsg += "check. sqlmap will display partial output"
        logger.warning(warnMsg)
    finally:
        kb.bruteMode = False
