...
def _goInference(payload, expression, charsetType=None, firstChar=None, lastChar=None, dump=False, field=None):
    ...
    if timeBasedCompare and conf.threads > 1 and kb.forceThreads is None:
        msg = "在基於時間的數據提取中使用多線程被認為是不安全的。你確定要這樣做嗎？（這將失去穩定性保障）[y/N] "
    ...
    if not (timeBasedCompare and kb.dnsTest):
        ...
        if not kb.bruteMode:
            debugMsg = "在%.2f 秒內執行了%d 個查詢" % (count, 'y' if count == 1 else "ies", calculateDeltaSeconds(start))
...
def _goInferenceProxy(expression, fromUser=False, batch=False, unpack=True, charsetType=None, firstChar=None, lastChar=None, dump=False):
    ...
    if len(expressionFieldsList) > 1:
        infoMsg = "提供的 SQL 查詢包含多個字段。sqlmap 將會把它拆分為多個獨立的查詢，以便即使在盲注的情況下也能獲取結果。"
            ...
            if test:
                ...
                if not stopLimit:
                    ...
                    if isNumPosStrValue(count):
                        ...
                        if batch or count == 1:
                            ...
                            message = "提供的 SQL 查詢可以返回 %d 條結果。你想要檢索多少條結果？\n" % count
                            message += "[a] 全部 (默認)\n[#] 指定數量\n"
                            message += "[q] 退出"
                            ...
                            if choice == 'A':
                                ...
                                infoMsg = "sqlmap 現在將檢索前%d 個查詢輸出條目" % stopLimit
                        ...
                        warnMsg = "無法計算提供的 SQL 查詢的條目數量。sqlmap 將假設它只返回一個條目"
                ...
                try:
                    try:
                        ...
                        errMsg = "邊界限制 (%d,%d) 過大。請使用'--fresh-queries'開關重新運行。"
                        #errMsg += "with switch '--fresh-queries'"
                        raise SqlmapDataException(errMsg)

                except KeyboardInterrupt:
                    print()
                    warnMsg = "用戶在轉儲階段中中止了操作"
                    logger.warning(warnMsg)
...
def getValue(expression, blind=True, union=True, error=True, time=True, fromUser=False, expected=None, batch=False, unpack=True, resumeValue=True, charsetType=None, firstChar=None, lastChar=None, dump=False, suppressOutput=None, expectingNone=False, safeCharEncode=True):
    ...
    if conf.hexConvert and expected != EXPECTED.BOOL and Backend.getIdentifiedDbms():
        if not hasattr(queries[Backend.getIdentifiedDbms()], "hex"):
            warnMsg = "在 DBMS %s 上,當前不支持'--hex'開關" % Backend.getIdentifiedDbms()
        ...
        if conf.direct:
            ...
            if not conf.forceDns:
                ...
                if found and conf.dnsDomain:
                    ...
                    warnMsg = "選項'--dns-domain'將被忽略,因為存在更快的可用技術 (%s)" % _
    ...
    if not any((kb.testMode, conf.dummy, conf.offline, conf.noCast, conf.hexConvert)) and value is None and Backend.getDbms() and conf.dbmsHandler and kb.fingerprinted:
        if conf.abortOnEmpty:
            errMsg = "由於無法檢索到數據,中止執行"
            logger.critical(errMsg)
            raise SystemExit
        else:
            warnMsg = "如果持續遇到數據檢索問題,建議嘗試使用開關'--no-cast' 或開關'--hex'" if hasattr(queries[Backend.getIdentifiedDbms()], "hex") else "如果持續遇到數據檢索問題,建議嘗試使用開關'--no-cast'"
