...
class Connect(object):
    ...
    def _retryProxy(**kwargs):
        ...
        if kb.testMode and kb.previousMethod == PAYLOAD.METHOD.TIME:
            ...
            warnMsg = "很可能 Web 服務器實例尚未從先前的基於時間的負載恢復。如果問題仍然存在,請等待幾分鐘後重新運行,不要在選項'--technique'中使用標誌'T'(例如'--flush-session --technique=BEUS'),或嘗試降低選項'--time-sec'的值 (例如'--time-sec=2')"
            ...
            if conf.tor:
                warnMsg = "請確保您已安裝並運行 Tor,以便成功使用'--tor'開關"
                ...
                warnMsg = "如果問題仍然存在,請檢查提供的目標 URL 是否可訪問"
                #warnMsg += "target URL is reachable"

                items = []
                if not conf.randomAgent:
                    items.append("開關 '--random-agent'")
                if not any((conf.proxy, conf.proxyFile, conf.tor)):
                    items.append("代理開關 ('--proxy', '--proxy-file'...)")
                if items:
                    warnMsg += "。如果是這樣,請嘗試使用" + "和/或".join(items) + "重新運行"
            ...
            warnMsg = "如果問題仍然存在,請嘗試降低使用的線程數 (選項'--threads')"
    ...
    def getPage(**kwargs):
        ...
        if conf.liveCookies:
            with kb.locks.liveCookies:
                if not checkFile(conf.liveCookies, raiseOnError=False) or os.path.getsize(conf.liveCookies) == 0:
                    warnMsg = "[%s] [警告] 實時 Cookie 文件'%s'為空或不存在。等待超時 (%d 秒)" % (time.strftime("%X"), conf.liveCookies, LIVE_COOKIES_TIMEOUT)
                    ...
                    if not valid:
                        errMsg = "從文件'%s'加載 Cookie 時出現問題" % conf.liveCookies
        ...
        try:
            ...
            if headers.get(HTTP_HEADER.CONTENT_TYPE) == POST_HINT_CONTENT_TYPES[POST_HINT.MULTIPART]:
                warnMsg = "在'%s'頭中缺少'boundary 參數'。將嘗試重建" % HTTP_HEADER.CONTENT_TYPE
            ...
            if webSocket:
                ...
                for function in kb.preprocessFunctions:
                    try:
                        ...
                        errMsg = "運行預處理函數'%s'時發生錯誤 ('%s')" % (function.__name__, getSafeExString(ex))
                ...
                if conf.http2:
                    ...
                    if LooseVersion(httpx.__version__) < LooseVersion(MIN_HTTPX_VERSION):
                        raise SqlmapMissingDependence("檢測到 httpx 版本過舊（%s < %s）" % (httpx.__version__, MIN_HTTPX_VERSION))
            ...
            if not refreshing:
                ...
                if extractRegexResult(META_REFRESH_REGEX, page):
                    ...
                    debugMsg = "獲取到 HTML 的 meta refresh 頭部"
                ...
                if not refresh:
                    ...
                    if refresh:
                        debugMsg = "獲取到 JavaScript 重定向邏輯"
                        logger.debug(debugMsg)

                if refresh:
                    if kb.alwaysRefresh is None:
                        msg = "檢測到刷新意圖 (類似於登錄頁面的重定向) 到'%s'。您是否希望從現在開始應用它？[Y/n]"
            ...
            if conn and not conf.keepAlive:
                try:
                    ...
                    warnMsg = "在關閉連接時發生問題 ('%s')" % getSafeExString(ex)
                    logger.warning(warnMsg)

        except SqlmapConnectionException as ex:
            if conf.proxyList and not kb.threadException:
                warnMsg = "無法連接到目標 URL('%s')" % getSafeExString(ex)
            ...
            try:
                ...
                warnMsg = "在嘗試獲取錯誤頁面信息時連接超時 (%d)" % ex.code
            ...
            if code in conf.abortCode:
                errMsg = "由於檢測到 HTTP 狀態碼'%d',中止執行" % code
                singleTimeLogMessage(errMsg, logging.CRITICAL)
                raise SystemExit

            if ex.code not in (conf.ignoreCode or []):
                if ex.code == _http_client.UNAUTHORIZED:
                    errMsg = "未經授權,請嘗試提供正確的 HTTP 身份驗證類型和有效的憑據 (%d)。如果這是有意的,請嘗試通過提供選項'--ignore-code'的有效值重新運行"
                    ...
                    warnMsg = "關閉 HTTP 分塊傳輸編碼,因為目標站點似乎不支持它 (%d)" % code
                    ...
                    warnMsg = "目標將請求 URI 標記為過長。建議嘗試使用'--no-cast'和/或'--no-escape'開關"
                    ...
                    if raise404:
                        ...
                        debugMsg = "頁面未找到 (%d)" % code
                        singleTimeLogMessage(debugMsg, logging.DEBUG)
                elif ex.code == _http_client.GATEWAY_TIMEOUT:
                    if ignoreTimeout:
                        return None if not conf.ignoreTimeouts else "", None, None
                    else:
                        warnMsg = "無法連接到目標 URL(%d - %s)" % (ex.code, _http_client.responses[ex.code])
                        if threadData.retriesCount < conf.retries and not kb.threadException:
                            warnMsg += ".sqlmap 將重試該請求"
                            logger.critical(warnMsg)
                            return Connect._retryProxy(**kwargs)
                        elif kb.testMode:
                            logger.critical(warnMsg)
                    ...
                    debugMsg = "獲取到 HTTP 錯誤代碼:%d('%s')" % (code, status)
                    logger.debug(debugMsg)
            ...
            if checking:
                ...
                warnMsg = "使用了無效的 URL 地址 (%s)" % repr(url)
                raise SqlmapSyntaxException(warnMsg)
            elif any(_ in tbMsg for _ in ("forcibly closed", "Connection is already closed", "ConnectionAbortedError")):
                warnMsg = "連接被目標 URL 強制關閉"
            elif "timed out" in tbMsg:
                if kb.testMode and kb.testType not in (None, PAYLOAD.TECHNIQUE.TIME, PAYLOAD.TECHNIQUE.STACKED):
                    singleTimeWarnMessage("目標主機（或 WAF/IPS）可能正在丟棄“可疑”請求")
                    kb.droppingRequests = True
                warnMsg = "連接到目標 URL 超時"
            elif "Connection reset" in tbMsg:
                if not conf.disablePrecon:
                    singleTimeWarnMessage("由於連接重置,關閉預連接機制")
                    conf.disablePrecon = True

                if kb.testMode:
                    singleTimeWarnMessage("目標 (或 WAF/IPS) 可能正在重置'可疑'請求")
                    kb.droppingRequests = True
                warnMsg = "與目標 URL 的連接被重置"
            ...
            with kb.locks.connError:
                ...
                if kb.connErrorCounter >= MAX_CONSECUTIVE_CONNECTION_ERRORS and kb.choices.connError is None:
                    message = "與目標的連接似乎存在持續問題。您確定要繼續嗎？[y/N] "
            ...
            for function in kb.postprocessFunctions:
                try:
                    ...
                    errMsg = "運行後處理函數'%s'時發生錯誤 ('%s')" % (function.__name__, getSafeExString(ex))
                    #errMsg += "function '%s' ('%s')" % (function.__name__, getSafeExString(ex))
                    raise SqlmapGenericException(errMsg)
            ...
            for _ in (getattr(conn, "redcode", None), code):
                if _ is not None and _ in conf.abortCode:
                    errMsg = "由於檢測到 HTTP 狀態碼'%d',中止執行" % _
                    singleTimeLogMessage(errMsg, logging.CRITICAL)
                    raise SystemExit
    ...
    def queryPage(value=None, place=None, content=False, getRatioValue=False, silent=False, method=None, timeBasedCompare=False, noteResponseTime=True, auxHeaders=None, response=False, raise404=None, removeReflection=True, disableTampering=False, ignoreSecondOrder=False):
        ...
        if payload:
            ...
            if not disableTampering and kb.tamperFunctions:
                for function in kb.tamperFunctions:
                    ...
                    try:
                        ...
                        errMsg = "運行篡改函數'%s'時發生錯誤 ('%s')" % (function.__name__, getSafeExString(ex))
                        #errMsg += "function '%s' ('%s')" % (function.__name__, getSafeExString(ex))
                        raise SqlmapGenericException(errMsg)

                    if not isinstance(payload, six.string_types):
                        errMsg = "篡改函數'%s'返回無效的負載類型 ('%s')" % (function.__name__, type(payload))
                ...
                if (place in (PLACE.GET, PLACE.URI, PLACE.COOKIE) or place == PLACE.CUSTOM_HEADER and value.split(',')[0].upper() == HTTP_HEADER.COOKIE.upper()) and not conf.skipUrlEncode or place in (PLACE.POST, PLACE.CUSTOM_POST) and postUrlEncode:
                    ...
                    if place == PLACE.COOKIE or place == PLACE.CUSTOM_HEADER and value.split(',')[0].upper() == HTTP_HEADER.COOKIE.upper():
                        if kb.choices.cookieEncode is None:
                            msg = "你想要對 cookie 值進行 URL 編碼嗎 (實現特定)？%s" % ("[Y/n]" if not conf.url.endswith(".aspx") else "[y/N]")  # Reference: https://support.microsoft.com/en-us/kb/313282
            ...
            if conf.hpp:
                if not any(conf.url.lower().endswith(_.lower()) for _ in (WEB_PLATFORM.ASP, WEB_PLATFORM.ASPX)):
                    warnMsg = "HTTP 參數汙染只適用於 ASP(.NET) 目標"
                ...
                if place in (PLACE.GET, PLACE.POST):
                    ...
                    warnMsg = "HTTP 參數汙染僅適用於常規的 GET 和 POST 參數"
        ...
        if conf.csrfToken:
            ...
            for attempt in xrange(conf.csrfRetries + 1):
                ...
                if attempt > 0:
                    warnMsg = "無法在'%s'中找到反 CSRF 令牌'%s'" % (conf.csrfUrl or conf.url, conf.csrfToken._original)
                    warnMsg += ".sqlmap 將重試該請求"
                    logger.warning(warnMsg)
            ...
            if not token:
                errMsg = "無法在'%s'中找到反 CSRF 令牌'%s'" % (conf.csrfUrl or conf.url, conf.csrfToken._original)
                if not conf.csrfUrl:
                    errMsg += "。您可以嘗試通過為選項'--csrf-url'提供有效值重新運行"
        ...
        if timeBasedCompare and not conf.disableStats:
            if len(kb.responseTimes.get(kb.responseTimeMode, [])) < MIN_TIME_RESPONSES:
                ...
                if conf.tor:
                    warnMsg = "強烈建議在基於時間的注入中避免使用'--tor'開關,因為它具有固有的高延遲時間"
                    #warnMsg += "time-based injections because of inherent high latency time"
                    singleTimeWarnMessage(warnMsg)

                warnMsg = "[%s] [WARNING] %s 基於時間的比較需要%s 統計模型,請等待" % (time.strftime("%X"), "(區分大小寫) " if kb.responseTimeMode else "", "更大的" if len(kb.responseTimes) == 1 else "重置")
                ...
                warnMsg = "非常重要的是,在使用基於時間的負載時不要過度使用網絡連接,以防止潛在的中斷。"
            ...
            if not kb.laggingChecked:
                ...
                if deviation is not None and deviation > WARN_TIME_STDEV:
                    ...
                    warnMsg = "檢測到連接響應的延遲較大。請儘可能使用較高的'--time-sec'選項值 (例如 10 或更高)。"
        ...
        if pageLength is None:
            try:
                ...
                warnMsg = "網站返回了異常大的響應"
                if kb.testMode:
                    warnMsg += "在測試階段中,這是自定義 WAF/IPS 解決方案中的常見行為。"
        ...
        if message:
            ...
            singleTimeWarnMessage("檢測到可能的權限問題（'%s'）" % message)
